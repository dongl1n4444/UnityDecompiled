namespace TreeEditor
{
    ...
    public class TreeData : ScriptableObject
    {
        ...
        public TreeNode DuplicateNode(TreeNode n)
        {
            ...
            TreeNode node2 = this.AddNode(g, this.GetNode(n.parentID));
            this.CopyFields(n, node2);
            return node2;
        }
            ...
            if (node.spline != null)
            {
                ...
                if (list.Count >= 2)
                {
                    ...
                    for (int j = 0; j < (list.Count - 2); j++)
                    {
                        ...
                        Vector3 vector11 = vector8 - positionAtTime;
                        Vector3 normalized = vector11.normalized;
                        Vector3 vector13 = vector9 - positionAtTime;
                        Vector3 vector12 = vector13.normalized;
                        bool flag2 = false;
                        if (Vector3.Dot(normalized, vector12) >= num4)
                        {
                            flag2 = true;
                        }
        ...
        private void UpdateShadowTexture(Texture2D shadowTexture, int texWidth, int texHeight)
        {
            if (shadowTexture != null)
            {
                ...
                TextureImporterPlatformSettings defaultPlatformTextureSettings = atPath.GetDefaultPlatformTextureSettings();
                if (num != defaultPlatformTextureSettings.maxTextureSize)
                {
                    defaultPlatformTextureSettings.maxTextureSize = num;
                    atPath.mipmapEnabled = true;
                    atPath.SetPlatformTextureSettings(defaultPlatformTextureSettings);
        ...
        private bool UpdateTextures(TextureAtlas atlas, List<TreeMaterial> materials)
        {
            ...
            bool flag2 = ((((this.optimizedSolidMaterial.GetTexture("_MainTex") != null) && (this.optimizedSolidMaterial.GetTexture("_BumpSpecMap") != null)) && ((this.optimizedSolidMaterial.GetTexture("_TranslucencyMap") != null) && (this.optimizedCutoutMaterial.GetTexture("_MainTex") != null))) && ((this.optimizedCutoutMaterial.GetTexture("_ShadowTex") != null) && (this.optimizedCutoutMaterial.GetTexture("_BumpSpecMap") != null))) && ((bool) this.optimizedCutoutMaterial.GetTexture("_TranslucencyMap"));
            ...
            if ((this.materialHash == str) && flag2)
            {
            ...
            textured5.Apply();
            Texture2D textured6 = new Texture2D(1, 1, TextureFormat.ARGB32, false, true);
            textured6.SetPixel(0, 0, white);
            textured6.Apply();
            Texture2D textured7 = new Texture2D(1, 1, TextureFormat.ARGB32, false, true);
            textured7.SetPixel(0, 0, color);
            textured7.Apply();
            Texture2D textured8 = new Texture2D(1, 1, TextureFormat.ARGB32, false, true);
            textured8.SetPixel(0, 0, color3);
            textured8.Apply();
            Texture2D textured9 = textured8;
            Texture2D textured10 = new Texture2D(1, 1, TextureFormat.ARGB32, false, true);
                ...
                for (int k = 0; k < atlas.nodes.Count; k++)
                {
                    ...
                    switch (j)
                    {
                        case 0:
                            ...
                            {
                                diffuseColor.r = Mathf.LinearToGammaSpace(diffuseColor.r);
                            }
                            if (rgbTexture == null)
                            {
        ...
        private static Texture2D[] WriteOptimizedTextures(string treeAssetPath, Texture2D[] textures)
        {
            ...
            {
                TextureImporter atPath = AssetImporter.GetAtPath(strArray[j]) as TextureImporter;
                if (j == 0)
                {
                ...
                {
                    atPath.sRGBTexture = false;
                }
                AssetDatabase.WriteImportSettingsIfDirty(strArray[j]);
            }
            try
            {
            ...
            {
                textures[k] = AssetDatabase.LoadMainAssetAtPath(strArray[k]) as Texture2D;
            }
            return textures;
        }
    }
