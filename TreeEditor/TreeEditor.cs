namespace TreeEditor
{
    ...
    internal class TreeEditor : Editor
    {
        ...
        private HierachyNode dragNode = null;
        private HierachyNode dropNode = null;
        ...
        private static GUIContent[] BuildToolbarContent(string[] iconStrings, string[] contentStrings, int selection)
        {
            ...
            {
                string str = (selection != i) ? "" : " On";
                string text = TreeEditorHelper.ExtractLabel(TreeEditorHelper.GetUIString(contentStrings[i]));
                contentArray[i] = EditorGUIUtility.IconContent(iconStrings[i] + str, text);
            }
            return contentArray;
        }
        ...
        private Vector2 GUIMinMaxSlider(PropertyType prop, string contentID, Vector2 value, float minimum, float maximum, bool hasCurve)
        {
            ...
            bool changed = GUI.changed;
            EditorGUI.MinMaxSlider(GUILayoutUtility.GetRect(gUIContent, "Button"), gUIContent, ref vector.x, ref vector.y, minimum, maximum);
            if (changed != GUI.changed)
            {
                this.GUIHandlePropertyChange(prop);
            }
        ...
        private bool OnCheckHotkeys(TreeData treeData, bool checkFrameSelected)
        {
            switch (Event.current.type)
            {
                ...
                case EventType.ExecuteCommand:
                    ...
                    if ((Event.current.commandName == "FrameSelected") && checkFrameSelected)
                    {
                        this.FrameSelected(base.target as Tree);
                        Event.current.Use();
                    }
                    if (Event.current.commandName == "UndoRedoPerformed")
                    {
        ...
        private void OnDisable()
        {
            ...
            if (target != null)
            {
                EditorUtility.SetSelectedRenderState(target.GetComponent<Renderer>(), EditorSelectedRenderState.Highlight | EditorSelectedRenderState.Wireframe);
            }
        }
        ...
        private void OnEnable()
        {
            ...
            if (target != null)
            {
                ...
                if (treeData != null)
                {
                    ...
                    bool flag = !(s_SelectedGroup is TreeGroupRoot);
                    EditorUtility.SetSelectedRenderState(component, !flag ? (EditorSelectedRenderState.Highlight | EditorSelectedRenderState.Wireframe) : EditorSelectedRenderState.Hidden);
                }
            }
        }
                ...
                if (s_SelectedGroup != null)
                {
                    ...
                    if ((s_SelectedGroup != null) && (s_SelectedGroup.GetType() == typeof(TreeGroupBranch)))
                    {
                                ...
                                for (int n = 0; n < spline2.nodes.Length; n++)
                                {
                                    ...
                                    switch (editMode)
                                    {
                                        ...
                                        case EditMode.RotateNode:
                                            ...
                                            {
                                                <>f__mg$cache2 = new Handles.CapFunction(Handles.CircleHandleCap);
                                            }
                                            Handles.FreeMoveHandle(position, Quaternion.identity, size, Vector3.zero, <>f__mg$cache2);
                                        ...
                                        case EditMode.Freehand:
                                            ...
                                            {
                                                <>f__mg$cache3 = new Handles.CapFunction(Handles.CircleHandleCap);
                                            }
                                            Handles.FreeMoveHandle(position, Quaternion.identity, size, Vector3.zero, <>f__mg$cache3);
                                            if (((rawType == EventType.MouseDown) && (current.type == EventType.Used)) && (keyboardControl != GUIUtility.keyboardControl))
                                            {
                                    ...
                                    {
                                        <>f__mg$cache1 = new Handles.CapFunction(Handles.RectangleHandleCap);
                                    }
                                    position = Handles.FreeMoveHandle(position, Quaternion.identity, size, Vector3.zero, <>f__mg$cache1);
                                Label_0361:
                                    if (((rawType == EventType.MouseDown) && (current.type == EventType.Used)) && (keyboardControl != GUIUtility.keyboardControl))
                                    {
                                        this.SelectNode(node2, treeData);
                        ...
                        if (((s_SelectedPoint > 0) && (editMode == EditMode.MoveNode)) && (s_SelectedNode != null))
                        {
                            ...
                            Matrix4x4 matrixx8 = localToWorldMatrix * node5.matrix;
                            Vector3 vector9 = matrixx8.MultiplyPoint(node6.point);
                            ...
                            if (Tools.pivotRotation == PivotRotation.Local)
                            {
                                switch (rawType)
                                {
                                    ...
                                    case EventType.MouseDown:
                                        this.m_StartPointRotation = MathUtils.QuaternionFromMatrix(matrixx8) * node6.rot;
                                        break;
                                }
                            ...
                            if (GUI.changed)
                            {
                                Undo.RegisterCompleteObjectUndo(treeData, "Move");
                                s_SelectedGroup.Lock();
                                node6.point = matrixx8.inverse.MultiplyPoint(vector9);
                        ...
                        if (((s_SelectedPoint >= 0) && (editMode == EditMode.RotateNode)) && (s_SelectedNode != null))
                        {
                            ...
                            Matrix4x4 matrixx10 = localToWorldMatrix * node7.matrix;
                            if (this.m_TempSpline == null)
                            {
                                this.m_TempSpline = new TreeSpline(node7.spline);
                            }
                            Vector3 vector10 = matrixx10.MultiplyPoint(node8.point);
                            ...
                            if (GUI.changed)
                            {
                                ...
                                for (int num11 = s_SelectedPoint + 1; num11 < this.m_TempSpline.nodes.Length; num11++)
                                {
                                    ...
                                    vector11 = matrixx10.MultiplyVector(vector11);
                                    vector11 = (Vector3) (globalToolRotation * vector11);
                                    vector11 = matrixx10.inverse.MultiplyVector(vector11);
                    ...
                    if ((s_SelectedGroup != null) && (s_SelectedGroup.GetType() == typeof(TreeGroupLeaf)))
                    {
                        for (int num12 = 0; num12 < s_SelectedGroup.nodeIDs.Length; num12++)
                        {
                            ...
                            Matrix4x4 matrixx12 = localToWorldMatrix * node9.matrix;
                            Vector3 vector13 = matrixx12.MultiplyPoint(Vector3.zero);
                            ...
                            switch (editMode)
                            {
                                case EditMode.MoveNode:
                                    ...
                                    {
                                        <>f__mg$cache4 = new Handles.CapFunction(Handles.CircleHandleCap);
                                    }
                                    Handles.FreeMoveHandle(vector13, Quaternion.identity, num13, Vector3.zero, <>f__mg$cache4);
                                    if (((type == EventType.MouseDown) && (current.type == EventType.Used)) && (num14 != GUIUtility.keyboardControl))
                                    {
                                        this.SelectNode(node9, treeData);
                                        this.m_GlobalToolRotation = MathUtils.QuaternionFromMatrix(matrixx12);
                                        this.m_StartMatrix = matrixx12;
                                    ...
                                    if (GUI.changed)
                                    {
                                        ...
                                        else if (group2.GetType() == typeof(TreeGroupRoot))
                                        {
                                ...
                                case EditMode.RotateNode:
                                    ...
                                    {
                                        <>f__mg$cache5 = new Handles.CapFunction(Handles.CircleHandleCap);
                                    }
                                    Handles.FreeMoveHandle(vector13, Quaternion.identity, num13, Vector3.zero, <>f__mg$cache5);
                                    if (((type == EventType.MouseDown) && (current.type == EventType.Used)) && (num14 != GUIUtility.keyboardControl))
                                    {
                                        this.SelectNode(node9, treeData);
                                        this.m_GlobalToolRotation = MathUtils.QuaternionFromMatrix(matrixx12);
                                        this.m_StartMatrix = matrixx12;
                                        this.m_StartPointRotation = node9.rotation;
                                        this.m_LockedWorldPos = new Vector3(matrixx12.m03, matrixx12.m13, matrixx12.m23);
                                    }
        ...
        {
            TreeData treeData = GetTreeData(base.target as Tree);
            if (treeData != null)
            {
