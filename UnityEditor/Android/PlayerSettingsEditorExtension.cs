namespace UnityEditor.Android
{
    ...
    internal class PlayerSettingsEditorExtension : DefaultPlayerSettingsEditorExtension
    {
        ...
        private string m_InternalTemplate;
        ...
        private string m_ProguardUserFile;
        ...
        public override void OnEnable(PlayerSettingsEditor settingsEditor)
        {
            ...
            string[] components = new string[] { "Assets", "Plugins", "Android" };
            this.m_PluginPath = Paths.Combine(components);
            string[] textArray2 = new string[] { BuildPipeline.GetBuildToolsDirectory(BuildTarget.Android), "GradleTemplates", "mainTemplate.gradle" };
            this.m_InternalTemplate = Paths.Combine(textArray2);
            string[] textArray3 = new string[] { this.m_PluginPath, "mainTemplate.gradle" };
            this.m_CustomTemplate = Paths.Combine(textArray3);
            string[] textArray4 = new string[] { this.m_PluginPath, "proguard-user.txt" };
            this.m_ProguardUserFile = Paths.Combine(textArray4);
        }
        ...
        public override void PublishSectionGUI(float h, float kLabelFloatMinW, float kLabelFloatMaxW)
        {
            ...
            EditorGUILayout.Space();
            GUILayout.Label(k_Texts.build, EditorStyles.boldLabel, new GUILayoutOption[0]);
            int androidBuildSystem = (int) EditorUserBuildSettings.androidBuildSystem;
            if ((androidBuildSystem < 0) || (androidBuildSystem >= k_Texts.buildSystemOptions.Length))
            {
                androidBuildSystem = 0;
            }
            EditorUserBuildSettings.androidBuildSystem = (AndroidBuildSystem) EditorGUILayout.Popup(k_Texts.buildSystem, androidBuildSystem, k_Texts.buildSystemOptions, new GUILayoutOption[0]);
            if (EditorUserBuildSettings.androidBuildSystem != AndroidBuildSystem.Gradle)
            {
                GUI.enabled = false;
                EditorGUILayout.Toggle(k_Texts.customGradleTemplate, false, new GUILayoutOption[0]);
                GUILayout.Label(EditorGUIUtility.TempContent("").text, EditorStyles.label, new GUILayoutOption[0]);
                GUILayout.Label(k_Texts.minify, EditorStyles.boldLabel, new GUILayoutOption[0]);
                EditorGUILayout.Popup(k_Texts.release, 0, k_Texts.minifyOptions, new GUILayoutOption[0]);
                EditorGUILayout.Popup(k_Texts.debug, 0, k_Texts.minifyOptions, new GUILayoutOption[0]);
                GUI.enabled = true;
            }
            else
            {
                bool flag5 = File.Exists(this.m_CustomTemplate);
                bool flag6 = EditorGUILayout.Toggle(k_Texts.customGradleTemplate, flag5, new GUILayoutOption[0]);
                if (flag6 && !flag5)
                {
                    this.ToggleAsset(this.m_InternalTemplate, this.m_CustomTemplate, true);
                }
                else if (!flag6 && flag5)
                {
                    this.ToggleAsset(this.m_InternalTemplate, this.m_CustomTemplate, false);
                }
                GUILayout.Label(EditorGUIUtility.TempContent(!flag6 ? "" : this.m_CustomTemplate).text, EditorStyles.label, new GUILayoutOption[0]);
                flag5 = File.Exists(this.m_ProguardUserFile);
                flag6 = EditorGUILayout.Toggle(k_Texts.userProguardFile, flag5, new GUILayoutOption[0]);
                if (flag6 && !flag5)
                {
                    this.ToggleAsset("", this.m_ProguardUserFile, true);
                }
                else if (!flag6 && flag5)
                {
                    this.ToggleAsset("", this.m_ProguardUserFile, false);
                }
                GUILayout.Label(EditorGUIUtility.TempContent(!flag6 ? "" : this.m_ProguardUserFile).text, EditorStyles.label, new GUILayoutOption[0]);
                GUILayout.Label(k_Texts.minify, EditorStyles.boldLabel, new GUILayoutOption[0]);
                int androidReleaseMinification = (int) EditorUserBuildSettings.androidReleaseMinification;
                int androidDebugMinification = (int) EditorUserBuildSettings.androidDebugMinification;
                androidReleaseMinification = EditorGUILayout.Popup(k_Texts.release, androidReleaseMinification, k_Texts.minifyOptions, new GUILayoutOption[0]);
                androidDebugMinification = EditorGUILayout.Popup(k_Texts.debug, androidDebugMinification, k_Texts.minifyOptions, new GUILayoutOption[0]);
                EditorUserBuildSettings.androidReleaseMinification = (AndroidMinification) androidReleaseMinification;
                EditorUserBuildSettings.androidDebugMinification = (AndroidMinification) androidDebugMinification;
            }
            EditorGUILayout.Space();
        ...
        private void ToggleAsset(string originalFile, string assetFile, bool exist)
        {
            string path = assetFile + ".DISABLED";
            string directoryName = Path.GetDirectoryName(assetFile);
            if (exist)
            {
                if (File.Exists(path))
                {
                    AssetDatabase.MoveAsset(path, assetFile);
                }
                else
                {
            ...
            {
                AssetDatabase.MoveAsset(assetFile, path);
            }
        }
        ...
        private class Texts
        {
            ...
            public GUIContent buildSystem = EditorGUIUtility.TextContent("Build System");
            public GUIContent[] buildSystemOptions = new GUIContent[] { EditorGUIUtility.TextContent("Internal (Default)"), EditorGUIUtility.TextContent("Gradle (New)"), EditorGUIUtility.TextContent("ADT (Legacy)") };
            ...
            public GUIContent customGradleTemplate = EditorGUIUtility.TextContent("Custom Gradle Template|Use if you need to tweak the Gradle build process by providing your own changes to the gradle build file.");
            public GUIContent debug = EditorGUIUtility.TextContent("Debug");
            ...
            public GUIContent minify = EditorGUIUtility.TextContent("Minify|How you want your java code to be minified. NOTE: Minifcation can sometimes remove needed code and produce non working binaries.");
            public GUIContent[] minifyOptions = new GUIContent[] { EditorGUIUtility.TextContent("None"), EditorGUIUtility.TextContent("Proguard"), EditorGUIUtility.TextContent("Gradle (Experimental)") };
            ...
            public GUIContent release = EditorGUIUtility.TextContent("Release");
            ...
            public GUIContent userProguardFile = EditorGUIUtility.TextContent("User Proguard File");
