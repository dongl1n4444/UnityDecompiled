namespace UnityEditor
{
    ...
    internal class SceneViewRotation
    {
        ...
        private static string[] kMenuDirNames = new string[] { "Free", "Right", "Top", "Front", "Left", "Bottom", "Back", "", "Perspective" };
            ...
            for (int i = kDirectionRotations.Length - 1; i >= 0; i--)
            {
                ...
                if (((num3 > 0.0) || (sgn <= 0f)) && ((num3 <= 0.0) || (sgn >= 0f)))
                {
                    ...
                    if (sgn > 0f)
                    {
                        ...
                        if (Handles.Button(this.m_ViewDirectionControlIDs[i], (Vector3) (((direction * Vector3.forward) * size) * -1.2f), direction, size, size * 0.7f, <>f__mg$cache0) && (!view.in2DMode && !view.isRotationLocked))
                        {
                    ...
                    if (sgn < 0f)
                    {
                        ...
                        {
                            <>f__mg$cache1 = new Handles.CapFunction(Handles.ConeHandleCap);
                        }
                        if (Handles.Button(this.m_ViewDirectionControlIDs[i], (Vector3) (((direction * Vector3.forward) * size) * -1.2f), direction, size, size * 0.7f, <>f__mg$cache1) && (!view.in2DMode && !view.isRotationLocked))
                        {
                            this.ViewAxisDirection(view, i);
                        }
                    }
        ...
        private void DrawLabels(SceneView view)
        {
            ...
            if ((!view.in2DMode && !view.isRotationLocked) && GUI.Button(position, string.Empty, styles.viewLabelStyleLeftAligned))
            {
            ...
            if (Event.current.type == EventType.Repaint)
            {
                ...
                rect3.x += 37f - (num2 * 0.5f);
                rect3.x = Mathf.RoundToInt(rect3.x);
                for (int j = 0; (j < this.dirNameVisible.Length) && (j < kDirNames.Length); j++)
                {
                    if (j != index)
                    {
                        Color color = Handles.centerColor;
                        color.a *= this.dirNameVisible[j].faded * this.fadedRotationLock;
                        if (color.a > 0f)
                        {
                            GUI.color = color;
                            GUI.Label(rect3, kDirNames[j], styles.viewLabelStyleLeftAligned);
                        }
                    }
                }
                Color centerColor = Handles.centerColor;
                centerColor.a *= this.faded2Dgray * this.fadedVisibility;
                if (centerColor.a > 0f)
                {
                ...
                {
                    this.DrawIsoStatusSymbol(new Vector3(rect3.x - 8f, rect3.y + 8.5f, 0f), view, (1f - this.faded2Dgray) * this.fadedRotationLock);
                }
            }
        }
        ...
        private void DrawRotationLock(SceneView view)
        {
            ...
            if (centerColor.a > 0f)
            {
                ...
                GUI.color = centerColor;
                GUIContent content = !view.isRotationLocked ? styles.unlockedRotationIcon : styles.lockedRotationIcon;
                if (GUI.Button(position, content, styles.lockStyle) && !view.in2DMode)
                {
        ...
        internal void HandleContextClick(SceneView view)
        {
            if (!view.in2DMode && !view.isRotationLocked)
            {
                ...
                {
                    Rect rect3 = new Rect((view.position.width - 100f) + 22f, 22f, 56f, 56f);
        ...
        internal void OnGUI(SceneView view)
        {
            if (Mathf.Min(view.position.width, view.position.height) >= 100f)
            {
                ...
                {
                    <>f__mg$cache2 = new Handles.CapFunction(Handles.CubeHandleCap);
                }
                if ((Handles.Button(this.m_CenterButtonControlID, Vector3.zero, Quaternion.identity, size * 0.8f, size, <>f__mg$cache2) && !view.in2DMode) && !view.isRotationLocked)
                {
        ...
        {
            public GUIContent lockedRotationIcon = EditorGUIUtility.IconContent("LockIcon-On", "Lock Rotation|Click to unlock the rotation.");
            public GUIStyle lockStyle = new GUIStyle("label");
            public GUIContent unlockedRotationIcon = EditorGUIUtility.IconContent("LockIcon", "Lock Rotation|Click to lock the rotation in the current direction.");
