namespace UnityEditor
{
    ...
    internal static class MaskFieldGUI
    {
        ...
        internal static int DoMaskField(Rect position, int controlID, int mask, string[] flagNames, int[] flagValues, GUIStyle style)
        {
            int num;
            bool flag;
            return DoMaskField(position, controlID, mask, flagNames, flagValues, style, out num, out flag);
        }
        ...
        Label_00FF:
            ...
            if (((event2.type == EventType.MouseDown) && position.Contains(event2.mousePosition)) || EditorExtensionMethods.MainActionKeyForControl(event2, controlID))
            {
                ...
                event2.Use();
                EditorUtility.DisplayCustomMenu(position, list2.ToArray(), !EditorGUI.showMixedValue ? list.ToArray() : new int[0], new EditorUtility.SelectMenuItemFunction(MaskCallbackInfo.m_Instance.SetMaskValueDelegate), flagValues);
