    ...
    public class NavMeshEditorHelpers
    {
        public static void DrawAgentDiagram(Rect rect, float agentRadius, float agentHeight, float agentClimb, float agentSlope)
        {
            if (Event.current.type == EventType.Repaint)
            {
                ...
                float num6 = 20f;
                float num7 = 10f;
                float b = rect.height - (num6 + num7);
                num4 = Mathf.Min((float) (b / (num2 + ((num * 2f) * num5))), (float) (b / (num * 2f)));
                num2 *= num4;
                num *= num4;
                a *= num4;
                a = Mathf.Min(a, b - (num * num5));
                ...
                for (int i = 0; i < 20; i++)
                {
                    ...
                    vectorArray3[i] = new Vector3(x - (num13 * num), (y - a) + ((num14 * num) * num5), 0f);
                }
                Color color = Handles.color;
                float xMin = rect.xMin;
                float num16 = y - a;
                float num17 = x - (b * 0.75f);
                float num18 = y;
                float num19 = x + (b * 0.75f);
                float num20 = y;
                float num21 = num19;
                float num22 = num20;
                ...
                Vector3[] vectorArray4 = new Vector3[] { new Vector3(xMin, y, 0f), new Vector3(num19 + num23, y, 0f) };
                ...
                GUI.Label(new Rect((x + num) + 5f, (y - (num2 * 0.5f)) - 10f, 150f, 20f), $"H = {agentHeight}");
                GUI.Label(new Rect(x, ((y - num2) - (num * num5)) - 15f, 150f, 20f), $"R = {agentRadius}");
