namespace UnityEditor
{
    ...
    internal class BoxEditor
    {
        ...
        {
            this.m_UseLossyScale = false;
            this.m_AlwaysDisplayHandles = false;
            this.m_AllowNegativeSize = true;
            this.drawMethodForHandles = null;
            this.getHandleSizeMethod = null;
        ...
        private Vector3 EdgeHandle(Vector3 handlePos, Vector3 handleDir, Vector3 slideDir1, Vector3 slideDir2, Matrix4x4 transform)
        {
            ...
            if (Camera.current != null)
            {
                ...
                Vector3 vector3 = handlePos - vector;
            ...
            if (alphaFactor > 0f)
            {
                ...
                {
                    <>f__mg$cache2 = new Handles.DrawCapFunction(Handles.DotCap);
                }
                handlePos = Slider2D.Do(controlID, handlePos, handleDir, slideDir1, slideDir2, HandleUtility.GetHandleSize(handlePos) * 0.03f, <>f__mg$cache2, SnapSettings.scale, true);
            }
            Handles.color = color;
            return handlePos;
        }
        ...
        private Vector3 MidpointHandle(Vector3 localPos, Vector3 localTangent, Vector3 localBinormal, Matrix4x4 transform, bool isCameraInsideBox)
        {
            ...
            if (alphaFactor > 0f)
            {
                ...
                else
                {
                    ...
                    {
                        <>f__mg$cache0 = new Handles.DrawCapFunction(BoxEditor.DefaultMidPointDrawFunc);
                    }
                    drawMethodForHandles = <>f__mg$cache0;
                }
