namespace UnityEditor
{
    ...
    internal class ColorPicker : EditorWindow
    {
        ...
        private string[] m_ColorBoxXAxisLabels = new string[] { "Saturation", "Hue", "Hue", "Blue", "Blue", "Red", "" };
        private string[] m_ColorBoxYAxisLabels = new string[] { "Brightness", "Brightness", "Saturation", "Green", "Red", "Green", "" };
        private string[] m_ColorBoxZAxisLabels = new string[] { "Hue", "Saturation", "Brightness", "Red", "Green", "Blue", "" };
        ...
        public static Texture2D GetGradientTextureWithAlpha0To1()
        {
            ...
            {
                textured = s_RightGradientTexture;
            }
            else
            {
                textured = s_RightGradientTexture = CreateGradientTexture("ColorPicker_0To1_Gradient", 4, 4, new Color(1f, 1f, 1f, 0f), new Color(1f, 1f, 1f, 1f));
            }
            return textured;
        }
        ...
        public static Texture2D GetGradientTextureWithAlpha1To0()
        {
            ...
            {
                textured = s_LeftGradientTexture;
            }
            else
            {
                textured = s_LeftGradientTexture = CreateGradientTexture("ColorPicker_1To0_Gradient", 4, 4, new Color(1f, 1f, 1f, 1f), new Color(1f, 1f, 1f, 0f));
            }
            return textured;
        }
        ...
        private void HandleCopyPasteEvents()
        {
            ...
            switch (current.type)
            {
                case EventType.ValidateCommand:
                {
                    ...
                    if ((commandName != null) && ((commandName == "Copy") || (commandName == "Paste")))
                    {
                        current.Use();
                    }
                    break;
        ...
        private Vector2 Slider2D(Rect rect, Vector2 value, Vector2 maxvalue, Vector2 minvalue, GUIStyle backStyle, GUIStyle thumbStyle)
        {
            if (backStyle != null)
            {
                ...
                int controlID = GUIUtility.GetControlID(s_Slider2Dhash, FocusType.Passive);
                ...
                Vector2 vector2 = new Vector2(((rect.width - (backStyle.padding.right + backStyle.padding.left)) - (width * 2f)) / (maxvalue.x - minvalue.x), ((rect.height - (backStyle.padding.top + backStyle.padding.bottom)) - (height * 2f)) / (maxvalue.y - minvalue.y));
                Rect position = new Rect((((rect.x + (value.x * vector2.x)) + (width / 2f)) + backStyle.padding.left) - (minvalue.x * vector2.x), (((rect.y + (value.y * vector2.y)) + (height / 2f)) + backStyle.padding.top) - (minvalue.y * vector2.y), width, height);
                Event current = Event.current;
                switch (current.GetTypeForControl(controlID))
                {
                    case EventType.MouseDown:
                        if (rect.Contains(current.mousePosition))
                        {
                            ...
                            GUIUtility.keyboardControl = 0;
                            value.x = ((((current.mousePosition.x - rect.x) - width) - backStyle.padding.left) / vector2.x) + minvalue.x;
                            value.y = ((((current.mousePosition.y - rect.y) - height) - backStyle.padding.top) / vector2.y) + minvalue.y;
                            GUI.changed = true;
                            Event.current.Use();
                        }
                        return value;
        ...
        private void SpecialHDRBrightnessHandling(Rect boxPos, Rect sliderPos)
        {
            ...
            if (this.m_ColorBoxMode == ColorBoxMode.HS_V)
            {
                ...
                EditorGUI.BeginChangeCheck();
                float num3 = EditableAxisLabel(rect3, rect5, this.m_HDRValues.m_HDRScaleFactor, 1f, this.m_HDRConfig.maxBrightness, styles.axisLabelNumberField);
                if (EditorGUI.EndChangeCheck())
                {
                    this.SetMaxDisplayBrightness(num3);
                }
            }
        }
        ...
        private float TexturedSlider(Texture2D background, string text, float val, float min, float max, float displayScale, string formatString, Action<float> onFloatFieldChanged)
        {
            ...
            int id = GUIUtility.GetControlID(0xd42b5, FocusType.Keyboard, base.position);
            EditorGUI.BeginChangeCheck();
