    ...
    internal class ConsoleWindow : EditorWindow, IHasCustomMenu
    {
        ...
        public void AddItemsToMenu(GenericMenu menu)
        {
            if (Application.platform == RuntimePlatform.OSXEditor)
            {
                ...
                {
                    <>f__mg$cache0 = new GenericMenu.MenuFunction(InternalEditorUtility.OpenPlayerConsole);
                }
                menu.AddItem(new GUIContent("Open Player Log"), false, <>f__mg$cache0);
            }
            if (<>f__mg$cache1 == null)
            {
                <>f__mg$cache1 = new GenericMenu.MenuFunction(InternalEditorUtility.OpenEditorConsole);
            }
            menu.AddItem(new GUIContent("Open Editor Log"), false, <>f__mg$cache1);
        ...
        private void AddStackTraceLoggingMenu(GenericMenu menu)
        {
            ...
            try
            {
                ...
                {
                    LogType current = (LogType) enumerator.Current;
                    ...
                    try
                    {
                        while (enumerator2.MoveNext())
                        {
                            ...
                            StackTraceLogType type2 = (StackTraceLogType) enumerator2.Current;
                            data.logType = current;
                            data.stackTraceLogType = type2;
                            object[] objArray1 = new object[] { "Stack Trace Logging/", current, "/", type2 };
                            menu.AddItem(new GUIContent(string.Concat(objArray1)), PlayerSettings.GetStackTraceLogType(current) == type2, new GenericMenu.MenuFunction2(this.ToggleLogStackTraces), data);
                        }
                    }
                    finally
                    {
            ...
            int stackTraceLogType = (int) PlayerSettings.GetStackTraceLogType(LogType.Log);
            IEnumerator enumerator3 = Enum.GetValues(typeof(LogType)).GetEnumerator();
            try
            {
                while (enumerator3.MoveNext())
                {
                    LogType logType = (LogType) enumerator3.Current;
                    if (PlayerSettings.GetStackTraceLogType(logType) != stackTraceLogType)
                    {
                        stackTraceLogType = -1;
                        goto Label_0176;
                    }
        ...
        Label_0176:
            enumerator4 = Enum.GetValues(typeof(StackTraceLogType)).GetEnumerator();
            try
            {
                while (enumerator4.MoveNext())
                {
                    StackTraceLogType userData = (StackTraceLogType) enumerator4.Current;
                    menu.AddItem(new GUIContent("Stack Trace Logging/All/" + userData), stackTraceLogType == userData, new GenericMenu.MenuFunction2(this.ToggleLogStackTracesForAll), userData);
                }
            }
            finally
            {
        ...
        private void OnGUI()
        {
            ...
            bool flag = HasFlag(ConsoleFlags.Collapse);
            SetFlag(ConsoleFlags.Collapse, GUILayout.Toggle(flag, "Collapse", Constants.MiniButtonLeft, new GUILayoutOption[0]));
            if (flag != HasFlag(ConsoleFlags.Collapse))
            {
            ...
            if (this.m_DevBuild)
            {
                ...
                SetFlag(ConsoleFlags.StopForAssert, GUILayout.Toggle(HasFlag(ConsoleFlags.StopForAssert), "Stop for Assert", Constants.MiniButtonLeft, new GUILayoutOption[0]));
                SetFlag(ConsoleFlags.StopForError, GUILayout.Toggle(HasFlag(ConsoleFlags.StopForError), "Stop for Error", Constants.MiniButtonRight, new GUILayoutOption[0]));
            }
            GUILayout.FlexibleSpace();
            int errorCount = 0;
            int warningCount = 0;
            ...
            bool val = GUILayout.Toggle(HasFlag(ConsoleFlags.LogLevelLog), new GUIContent((logCount > 0x3e7) ? "999+" : logCount.ToString(), (logCount <= 0) ? iconInfoMono : iconInfoSmall), Constants.MiniButtonRight, new GUILayoutOption[0]);
            bool flag4 = GUILayout.Toggle(HasFlag(ConsoleFlags.LogLevelWarning), new GUIContent((warningCount > 0x3e7) ? "999+" : warningCount.ToString(), (warningCount <= 0) ? iconWarnMono : iconWarnSmall), Constants.MiniButtonMiddle, new GUILayoutOption[0]);
            bool flag5 = GUILayout.Toggle(HasFlag(ConsoleFlags.LogLevelError), new GUIContent((errorCount > 0x3e7) ? "999+" : errorCount.ToString(), (errorCount <= 0) ? iconErrorMono : iconErrorSmall), Constants.MiniButtonLeft, new GUILayoutOption[0]);
            SetFlag(ConsoleFlags.LogLevelLog, val);
            SetFlag(ConsoleFlags.LogLevelWarning, flag4);
            SetFlag(ConsoleFlags.LogLevelError, flag5);
            ...
            {
                bool flag6 = false;
                bool flag7 = HasFlag(ConsoleFlags.Collapse);
        ...
        public void ToggleLogStackTracesForAll(object userData)
        {
            ...
            try
            {
                ...
                {
                    LogType current = (LogType) enumerator.Current;
                    PlayerSettings.SetStackTraceLogType(current, (StackTraceLogType) userData);
                }
            }
            finally
            {
