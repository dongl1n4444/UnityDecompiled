namespace UnityEditor
{
    ...
    internal class AnnotationWindow : EditorWindow
    {
        ...
        private void DrawAnnotationList(float startY, float height)
        {
            ...
            this.DrawNormalList(true, width, this.m_ScrollPosition.y - 18f, this.m_ScrollPosition.y + num);
            GUI.EndScrollView();
        }
        ...
        private void DrawListElement(Rect rect, bool even, AInfo ainfo)
        {
            ...
            else
            {
                ...
                if (ainfo.HasGizmo())
                {
                    ...
                    ainfo.m_GizmoEnabled = GUI.Toggle(rect6, ainfo.m_GizmoEnabled, new GUIContent("", str), m_Styles.toggle);
                    if (GUI.changed)
                    {
        ...
        private float DrawListSection(float y, string sectionHeader, List<AInfo> listElements, bool doDraw, float listElementWidth, float startY, float endY, ref bool even, bool useSeperator, ref bool headerDrawn)
        {
            ...
            if (doDraw)
            {
                ...
                GUI.Label(new Rect(1f, num, listElementWidth - 2f, height), GUIContent.Temp(""), style2);
                GUI.Label(new Rect(10f, num + 3f, listElementWidth - 15f, 3f), GUIContent.Temp(""), m_Styles.seperator);
            }
            return (num + height);
        }
        ...
        private void DrawTopSection(float topSectionHeight)
        {
            ...
            float width = 120f;
            float height = 20f;
            Rect position = new Rect(num2 - 2f, y, width, height);
            ...
            if (s_Debug)
            {
                Rect rect3 = new Rect(0f, y + 10f, base.position.width - num2, height);
                GUI.Label(rect3, ConvertTexelWorldSizeToString(iconSize), m_Styles.texelWorldSizeStyle);
            }
            using (new EditorGUI.DisabledScope(!AnnotationUtility.use3dGizmos))
            {
                float num7 = (base.position.width - num2) - width;
                float num8 = ConvertTexelWorldSizeTo01(iconSize);
                Rect rect6 = new Rect((base.position.width - num2) - num7, y, num7, height);
                num8 = GUI.HorizontalSlider(rect6, num8, 0f, 1f);
                if (GUI.changed)
                {
                    AnnotationUtility.iconSize = Convert01ToTexelWorldSize(num8);
            ...
            {
                position = new Rect(num2 - 2f, y, width, height);
        ...
        {
            <GetAInfo>c__AnonStorey0 storey = new <GetAInfo>c__AnonStorey0 {
                scriptClass = scriptClass,
                classID = classID
            };
            if (storey.scriptClass != "")
            {
                return this.m_ScriptAnnotations.Find(new Predicate<AInfo>(storey.<>m__0));
            }
        ...
        [CompilerGenerated]
        private sealed class <GetAInfo>c__AnonStorey0
        {
