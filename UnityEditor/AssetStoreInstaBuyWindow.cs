namespace UnityEditor
{
    ...
    internal class AssetStoreInstaBuyWindow : EditorWindow
    {
        ...
        private PurchaseStatus m_Purchasing = PurchaseStatus.Init;
        ...
        {
            <DownloadPackage>c__AnonStorey0 storey = new <DownloadPackage>c__AnonStorey0 {
                $this = this,
            ...
            AssetStoreContext.Download(this.m_Asset.packageID.ToString(), storey.item.packageUrl, storey.item.encryptionKey, storey.item.packageName, storey.item.publisherName, storey.item.categoryName, new AssetStoreUtils.DownloadDoneCallback(storey.<>m__0));
        }
        ...
        {
            AssetStoreInstaBuyWindow window = ShowAssetStoreInstaBuyWindow(asset, "", "", "", "");
            if (window.m_Purchasing != PurchaseStatus.Init)
            {
                EditorUtility.DisplayDialog("Download in progress", "There is already a package download in progress. You can only have one download running at a time", "Close");
            }
        ...
        [CompilerGenerated]
        private sealed class <DownloadPackage>c__AnonStorey0
        {
            internal AssetStoreInstaBuyWindow $this;
            internal AssetStoreAsset.PreviewInfo item;

            internal void <>m__0(string id, string status, int bytes, int total)
            {
                if (this.$this != null)
                {
                    ...
                    {
                        if (((this.$this.m_Asset == null) || (this.$this.m_Purchasing != AssetStoreInstaBuyWindow.PurchaseStatus.Downloading)) || (id != this.$this.m_Asset.packageID.ToString()))
                        {
                            this.$this.Close();
                        }
