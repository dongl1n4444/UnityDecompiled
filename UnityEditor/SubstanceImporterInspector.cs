namespace UnityEditor
{
    ...
    internal class SubstanceImporterInspector : Editor
    {
        ...
        {
            AssetDatabase.ImportAsset(AssetDatabase.GetAssetPath(base.target), ImportAssetOptions.ForceUncompressedImport);
            if (exitGUI)
            {
                GUIUtility.ExitGUI();
            }
        ...
        private void MaterialManagement()
        {
            ...
            using (new EditorGUI.DisabledScope(EditorApplication.isPlaying))
            {
                if (this.m_PrototypeNames.Length > 1)
                {
                    ...
                    if (EditorGUI.DropdownButton(position, this.m_SubstanceStyles.iconToolbarPlus, FocusType.Passive, this.m_SubstanceStyles.toolbarDropDown))
                    {
