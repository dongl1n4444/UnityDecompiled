namespace UnityEditor
{
    ...
    internal class ExposablePopupMenu
    {
        ...
        private Action<ItemData> m_SelectionChangedCallback = null;
            ...
            internal static void Show(Rect activatorRect, List<ExposablePopupMenu.ItemData> buttonData, ExposablePopupMenu caller)
            {
                ...
                foreach (ExposablePopupMenu.ItemData data in m_Data)
                {
                    if (data.m_Enabled)
                    {
                        ...
                        {
                            <>f__mg$cache0 = new GenericMenu.MenuFunction2(ExposablePopupMenu.PopUpMenu.SelectionCallback);
                        }
                        menu.AddItem(data.m_GUIContent, data.m_On, <>f__mg$cache0, data);
                    }
                    else
                    {
