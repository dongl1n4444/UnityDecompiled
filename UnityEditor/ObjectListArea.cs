namespace UnityEditor
{
    ...
    {
        [CompilerGenerated]
        private static Func<KeyValuePair<string, int>, string> <>f__am$cache0;
        [CompilerGenerated]
        private static Predicate<AssetStoreGroup> <>f__am$cache1;
        ...
        public ObjectListArea(ObjectListAreaState state, EditorWindow owner, bool showNoneItem)
        {
            ...
            this.m_LocalAssets = new LocalGroup(this, "", showNoneItem);
        ...
        public void BeginPing(int instanceID)
        {
            ...
            if (index != -1)
            {
                <BeginPing>c__AnonStorey4 storey = new <BeginPing>c__AnonStorey4();
                ...
                if (fullText != null)
                {
                    ...
                    {
                        <BeginPing>c__AnonStorey3 storey2 = new <BeginPing>c__AnonStorey3 {
                            <>f__ref$4 = storey
                        };
                    ...
                    else
                    {
                        ...
                        this.m_Ping.m_ContentDraw = new Action<Rect>(storey.<>m__0);
                    }
            ...
            switch (selectionOffset)
            {
                case 0x7ffffffe:
                    ...
                    else
                    {
                        ...
                        int num4 = maxIndex - idx;
                        this.m_SelectionOffset = Mathf.Min(Mathf.FloorToInt(((float) num4) / ((float) columns)) * columns, this.m_SelectionOffset);
                    }
                    break;
        ...
        public string[] GetCurrentVisibleNames()
        {
            ...
            {
                <>f__am$cache0 = x => x.Key;
            }
            return Enumerable.Select<KeyValuePair<string, int>, string>(this.m_LocalAssets.GetVisibleNameAndInstanceIDs(), <>f__am$cache0).ToArray<string>();
        }
        ...
        private void HandleListArea()
        {
            ...
            if (!this.IsObjectSelector() && !this.m_QueryInProgress)
            {
                ...
                {
                    <>f__am$cache1 = g => g.NeedItems;
                }
                if (this.m_StoreAssets.Exists(<>f__am$cache1) || this.m_RequeryAssetStore)
                {
        ...
        {
            <QueryAssetStore>c__AnonStorey0 storey = new <QueryAssetStore>c__AnonStorey0 {
                $this = this
            };
            ...
            {
                bool flag2 = ((this.m_LastAssetStoreQuerySearchFilter != "") || (this.m_LastAssetStoreQueryClassName.Length != 0)) || (this.m_LastAssetStoreQueryLabels.Length != 0);
                if (!this.m_QueryInProgress)
                {
                    ...
                    else
                    {
                        ...
                        AssetStoreResultBase<AssetStoreSearchResults>.Callback callback = new AssetStoreResultBase<AssetStoreSearchResults>.Callback(storey.<>m__0);
        ...
        [CompilerGenerated]
        private sealed class <BeginPing>c__AnonStorey3
        {
            internal ObjectListArea.<BeginPing>c__AnonStorey4 <>f__ref$4;
            internal FilteredHierarchy.FilterResult res;

            internal void <>m__0(Rect r)
            {
                ObjectListArea.LocalGroup.DrawIconAndLabel(r, this.res, this.<>f__ref$4.label, this.<>f__ref$4.hierarchyProperty.icon, false, false);
            }
        }

        [CompilerGenerated]
        private sealed class <BeginPing>c__AnonStorey4
        {
        ...
        [CompilerGenerated]
        private sealed class <QueryAssetStore>c__AnonStorey0
        {
            internal ObjectListArea $this;
            internal string queryFilter;

            internal void <>m__0(AssetStoreSearchResults results)
            {
                this.$this.m_QueryInProgress = false;
                if (this.queryFilter != (this.$this.m_LastAssetStoreQuerySearchFilter + this.$this.m_LastAssetStoreQueryClassName + this.$this.m_LastAssetStoreQueryLabels))
                {
                    this.$this.m_RequeryAssetStore = true;
                }
                if ((results.error != null) && (results.error != ""))
                {
                    ...
                    this.$this.m_AssetStoreError = results.error;
                    this.$this.m_Groups.Clear();
                    this.$this.m_Groups.Add(this.$this.m_LocalAssets);
                    this.$this.Repaint();
                ...
                {
                    this.$this.m_AssetStoreError = "";
                    List<string> list = new List<string>();
                    foreach (ObjectListArea.AssetStoreGroup group in this.$this.m_StoreAssets)
                    {
                        list.Add(group.Name);
                    }
                    this.$this.m_Groups.Clear();
                    ...
                    using (List<AssetStoreSearchResults.Group>.Enumerator enumerator2 = results.groups.GetEnumerator())
                    {
                        ...
                        {
                            <QueryAssetStore>c__AnonStorey1 storey = new <QueryAssetStore>c__AnonStorey1 {
                                <>f__ref$0 = this,
                                inGroup = enumerator2.Current
                            };
                            list.Remove(storey.inGroup.name);
                            ObjectListArea.AssetStoreGroup item = this.$this.m_StoreAssets.Find(new Predicate<ObjectListArea.AssetStoreGroup>(storey.<>m__0));
                            if (item == null)
                            {
                                item = new ObjectListArea.AssetStoreGroup(this.$this, storey.inGroup.label, storey.inGroup.name);
                                this.$this.m_StoreAssets.Add(item);
                            }
                            this.$this.m_Groups.Add(item);
                    ...
                    using (List<string>.Enumerator enumerator3 = list.GetEnumerator())
                    {
                        ...
                        {
                            <QueryAssetStore>c__AnonStorey2 storey2 = new <QueryAssetStore>c__AnonStorey2 {
                                <>f__ref$0 = this,
                                k = enumerator3.Current
                            };
                            this.$this.m_StoreAssets.RemoveAll(new Predicate<ObjectListArea.AssetStoreGroup>(storey2.<>m__0));
                        }
                    }
                    this.$this.EnsureAssetStoreGroupsAreOpenIfAllClosed();
                    this.$this.Repaint();
                    if (this.$this.assetStoreSearchEnded != null)
                    {
                        this.$this.assetStoreSearchEnded();
            ...
            private sealed class <QueryAssetStore>c__AnonStorey1
            {
            ...
            protected override void DrawInternal(int itemIdx, int endItem, float yOffset)
            {
                ...
                else
                {
                    while ((itemIdx < endItem) && (itemIdx < count))
                    {
                        ...
                        if (flag)
                        {
                            bool flag2 = !AssetStoreAssetSelection.Empty && AssetStoreAssetSelection.ContainsAsset(this.m_Assets[itemIdx].id);
                            this.DrawLabel(rect, this.m_Assets[itemIdx], flag2);
                        }
                        itemIdx++;
                    }
                }
        ...
        private abstract class Group
        {
            ...
            private int FirstVisibleRow(float yOffset, Vector2 scrollPos)
            {
                ...
                float num2 = scrollPos.y - (yOffset + this.GetHeaderHeight());
                int num3 = 0;
                if (num2 > 0f)
                {
                    float num4 = this.m_Grid.itemSize.y + this.m_Grid.verticalSpacing;
                    num3 = (int) Mathf.Max(0f, Mathf.Floor(num2 / num4));
                }
                return num3;
            }
        ...
        private class LocalGroup : ObjectListArea.Group
        {
            ...
            public static void DrawIconAndLabel(Rect rect, FilteredHierarchy.FilterResult filterItem, string label, Texture2D icon, bool selected, bool focus)
            {
                ...
                rect.xMin += ObjectListArea.s_Styles.resultsLabel.margin.left;
            ...
            private void DrawItem(Rect position, FilteredHierarchy.FilterResult filterItem, BuiltinResource builtinResource, bool isFolderBrowsing)
            {
                ...
                Rect rect2 = new Rect(position.x + ObjectListArea.s_Styles.groupFoldout.margin.left, position.y, (float) ObjectListArea.s_Styles.groupFoldout.padding.left, position.height);
                ...
                if ((flag2 && (current.type == EventType.KeyDown)) && base.m_Owner.HasFocus())
                {
                    ...
                    {
                        case KeyCode.LeftArrow:
                            if (this.ListMode || base.m_Owner.IsPreviewIconExpansionModifierPressed())
                            {
                ...
                if (Event.current.type == EventType.Repaint)
                {
                    ...
                    if (this.ListMode)
                    {
                        ...
                        DrawIconAndLabel(new Rect(xMax, position.y, position.width - xMax, position.height), filterItem, label, icon, flag2, base.m_Owner.HasFocus());
                        if (flag)
                        {
                    ...
                    else
                    {
                        ...
                        if (flag7)
                        {
                            Rect rect7 = new RectOffset(1, 1, 1, 1).Remove(ObjectListArea.s_Styles.textureIconDropShadow.border.Add(rect4));
                            ObjectListArea.s_Styles.textureIconDropShadow.Draw(rect7, GUIContent.none, false, false, flag2 || flag6, (base.m_Owner.HasFocus() || flag5) || flag6);
                        }
                        if (!flag5)
                        {
                            if (flag6)
                ...
                if (flag5)
                {
                    if (this.ListMode)
                    {
                        float num10 = ((num6 + 16f) + 2f) + ObjectListArea.s_Styles.resultsLabel.margin.left;
            ...
            protected void DrawSubAssetRowBg(int startSubAssetIndex, int endSubAssetIndex, bool continued, float yOffset)
            {
                ...
                Rect rect4 = new Rect((rect2.xMax - width) + num8, rect2.y + num5, width, position.height);
                rect4.y = Mathf.Round(rect4.y);
                rect4.height = Mathf.Ceil(rect4.height);
                ...
                position = new Rect(position.xMax, position.y, rect4.xMin - position.xMax, position.height);
                position.y = Mathf.Round(position.y);
                position.height = Mathf.Ceil(position.height);
            ...
            public FilteredHierarchy.FilterResult LookupByInstanceID(int instanceID)
            {
                ...
                {
                    int num = 0;
                    foreach (FilteredHierarchy.FilterResult result2 in this.m_FilteredHierarchy.results)
                    {
                        if (base.m_Owner.m_State.m_NewAssetIndexInList == num)
                        {
                            num++;
                        }
                        if (result2.instanceID == instanceID)
                        {
                            return result2;
                        }
                        num++;
            ...
            {
                string message = "";
                message = message + "ObjectSelector -Builtin Assets Available:\n";
                foreach (KeyValuePair<string, BuiltinResource[]> pair in this.m_BuiltinResourceMap)
                {
                    BuiltinResource[] resourceArray = pair.Value;
            ...
            private class ItemFader
            {
                ...
                public float GetAlpha(int instanceID)
                {
                    if (this.m_InstanceIDs != null)
                    {
                        ...
                        if (index >= 0)
                        {
                            double num3 = EditorApplication.timeSinceStartup - this.m_FadeStartTime;
                            double num4 = this.m_TimeBetweenEachItem * index;
                            float num5 = 0f;
                            if (num4 < num3)
                            {
                                num5 = Mathf.Clamp((float) ((num3 - num4) / this.m_FadeDuration), 0f, 1f);
                            }
                            return num5;
                        }
                    }
