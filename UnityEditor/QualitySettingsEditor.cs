namespace UnityEditor
{
    ...
    {
        [CompilerGenerated]
        private static Func<QualitySetting, string> <>f__am$cache0;
        public const int kMaxAsyncRingBufferSize = 0x200;
        public const int kMaxAsyncUploadTimeSlice = 0x21;
                ...
                if (((this.m_QualitySettingsProperty.arraySize > 1) && (this.m_DeleteLevel >= 0)) && (this.m_DeleteLevel < this.m_QualitySettingsProperty.arraySize))
                {
                    ...
                    foreach (string str in list)
                    {
                        ...
                        if ((num != 0) && (num >= this.m_DeleteLevel))
                        {
                            ...
                            string str2;
                            (dictionary = platformDefaults)[str2 = str] = dictionary[str2] - 1;
                        }
                    }
                }
        ...
        {
            foreach (QualitySetting setting in settings)
            {
                ...
                if (property != null)
                {
                    ...
                    if (property2.arraySize != setting.m_ExcludedPlatforms.Count)
                    {
                        property2.arraySize = setting.m_ExcludedPlatforms.Count;
                    }
                    int num = 0;
                    IEnumerator enumerator = property2.GetEnumerator();
                    try
                    {
                        while (enumerator.MoveNext())
                        {
                            SerializedProperty current = (SerializedProperty) enumerator.Current;
                            if (current.stringValue != setting.m_ExcludedPlatforms[num])
                            {
                                current.stringValue = setting.m_ExcludedPlatforms[num];
                            }
                            num++;
                        }
                    }
