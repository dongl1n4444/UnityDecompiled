namespace UnityEditor
{
    ...
    internal sealed class AssetStoreContext
    {
        ...
        public static string[] PackageStorePath(string publisher_name, string category_name, string package_name, string package_id, string url)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                strArray[i] = s_InvalidPathCharsRegExp.Replace(strArray[i], "");
            }
            if (strArray[2] == "")
            {
                strArray[2] = s_InvalidPathCharsRegExp.Replace(package_id, "");
            }
            if (strArray[2] == "")
            {
                strArray[2] = s_InvalidPathCharsRegExp.Replace(url, "");
            }
            return strArray;
        }
