namespace UnityEditor
{
    ...
    internal class ParentViewState
    {
        ...
        public void AddAssetItems(AssetsItem[] assets)
        {
            ...
            {
                <>f__mg$cache0 = new Comparison<ParentViewFolder>(ParentViewState.CompareViewFolder);
            }
            Array.Sort<ParentViewFolder>(this.folders, <>f__mg$cache0);
            for (int i = 0; i < this.folders.Length; i++)
            {
                if (<>f__mg$cache1 == null)
                {
                    <>f__mg$cache1 = new Comparison<ParentViewFile>(ParentViewState.CompareViewFile);
                }
                Array.Sort<ParentViewFile>(this.folders[i].files, <>f__mg$cache1);
            }
        }
        ...
        public void AddAssetItems(Changeset assets)
        {
            ...
            {
                <>f__mg$cache2 = new Comparison<ParentViewFolder>(ParentViewState.CompareViewFolder);
            }
            Array.Sort<ParentViewFolder>(this.folders, <>f__mg$cache2);
            for (int i = 0; i < this.folders.Length; i++)
            {
                if (<>f__mg$cache3 == null)
                {
                    <>f__mg$cache3 = new Comparison<ParentViewFile>(ParentViewState.CompareViewFile);
                }
                Array.Sort<ParentViewFile>(this.folders[i].files, <>f__mg$cache3);
            }
        }
        ...
        public void AddAssetItems(DeletedAsset[] assets)
        {
            ...
            {
                <>f__mg$cache4 = new Comparison<ParentViewFolder>(ParentViewState.CompareViewFolder);
            }
            Array.Sort<ParentViewFolder>(this.folders, <>f__mg$cache4);
            for (int i = 0; i < this.folders.Length; i++)
            {
                if (<>f__mg$cache5 == null)
                {
                    <>f__mg$cache5 = new Comparison<ParentViewFile>(ParentViewState.CompareViewFile);
                }
                Array.Sort<ParentViewFile>(this.folders[i].files, <>f__mg$cache5);
            }
        }
