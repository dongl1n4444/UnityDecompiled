    ...
    internal class RectTool : ManipulationTool
    {
        ...
        private static Vector3 PivotHandleGUI(Rect rect, Vector3 pivot, Quaternion rotation)
        {
            ...
            if ((GUI.color.a > 0f) || (GUIUtility.hotControl == controlID))
            {
                ...
                Vector3 vector = Handles.Slider2D(controlID, pivot, (Vector3) (rotation * Vector3.forward), (Vector3) (rotation * Vector3.right), (Vector3) (rotation * Vector3.up), HandleUtility.GetHandleSize(pivot) * 0.1f, <>f__mg$cache1, Vector2.zero);
                    ...
                    if ((i != 1) || (j != 1))
                    {
                        ...
                        if ((GUI.color.a > 0f) || (GUIUtility.hotControl == controlID))
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    <>f__mg$cache0 = new Handles.CapFunction(RectHandles.RectScalingHandleCap);
                                }
                                vector4 = RectHandles.CornerSlider(controlID, position, (Vector3) (rotation * Vector3.forward), vector7, vector8, size, <>f__mg$cache0, Vector2.zero);
                            }
                            bool flag = ((Selection.transforms.Length == 1) && InternalEditorUtility.SupportsRectLayout(Selection.activeTransform)) && (Selection.activeTransform.parent.rotation == rotation);
                            if (flag)
                            {
                            ...
                            if (EditorGUI.EndChangeCheck())
                            {
                                ...
                                if (flag)
                                {
                                    ...
                                    Vector3 vector10 = (Vector3) (quaternion * transform5.TransformVector(Vector3.right));
                                    snapDistance.x /= vector10.x;
                                    Vector3 vector11 = (Vector3) (quaternion * transform5.TransformVector(Vector3.up));
                                    snapDistance.y /= vector11.y;
                                    Vector3 vector12 = transform5.InverseTransformPoint(vector4) - transform6.rect.min;
                                    Vector3 positionAfterSnapping = (Vector3) (RectTransformSnapping.SnapToGuides(vector12, snapDistance) + (Vector3.forward * vector12.z));
                                    ManipulationToolUtility.DisableMinDragDifferenceBasedOnSnapping(vector12, positionAfterSnapping);
                                ...
                                Vector3 vector14 = (Vector3) (quaternion * (vector2 - scalePivot));
                                Vector3 vector15 = (Vector3) (quaternion * (vector4 - scalePivot));
                                if (i != 1)
                                {
                                    one.x = vector15.x / vector14.x;
                                }
                                if (j != 1)
                                {
                                    one.y = vector15.y / vector14.y;
                                }
                                if (flag4)
                                {
                                ...
                                if (flag4)
                                {
                                    float num7 = (i != 1) ? one.x : one.y;
                                    one = (Vector3) (Vector3.one * num7);
                                }
                                if (actionKey && (i == 1))
                                {
                                    if (Event.current.shift)
        ...
        public override void ToolGUI(SceneView view, Vector3 handlePosition, bool isStatic)
        {
            ...
            if (!Tools.vertexDragging)
            {
                ...
                {
                    EditorGUI.BeginChangeCheck();
                    Vector3 vector7 = PivotHandleGUI(handleRect, pivot, handleRectRotation);
                    if (EditorGUI.EndChangeCheck() && !isStatic)
                    {
                        if (flag)
                        {
                            Tools.localHandleOffset += Quaternion.Inverse(Tools.handleRotation) * (vector7 - pivot);
                        }
                        else if (flag2)
                        {
                            Transform activeTransform = Selection.activeTransform;
                            ...
                            Vector2 vector8 = transform3.InverseTransformVector(vector7 - pivot);
                            vector8.x /= component.rect.width;
                            vector8.y /= component.rect.height;
                            Vector2 vector9 = component.pivot + vector8;
                            RectTransformEditor.SetPivotSmart(component, vector9.x, 0, true, transform3 != component.transform);
                            RectTransformEditor.SetPivotSmart(component, vector9.y, 1, true, transform3 != component.transform);
                        }
                    }
                }
            }
            ...
            if (!Tools.vertexDragging)
            {
                ...
                if (flag3)
                {
                    ...
                    if (EditorGUI.EndChangeCheck() && !isStatic)
                    {
                        float num5;
                        Vector3 vector12;
                        (Quaternion.Inverse(handleRectRotation) * quaternion2).ToAngleAxis(out num5, out vector12);
                        vector12 = (Vector3) (handleRectRotation * vector12);
                        Undo.RecordObjects(Selection.transforms, "Rotate");
                        foreach (Transform transform5 in Selection.transforms)
                        {
                            transform5.RotateAround(handlePosition, vector12, num5);
                            transform5.SetLocalEulerHint(transform5.GetLocalEulerAngles(transform5.rotationOrder));
                            if (transform5.parent != null)
                            {
                                transform5.SendTransformChangedScale();
                            }
                        }
                        Tools.handleRotation = Quaternion.AngleAxis(num5, vector12) * Tools.handleRotation;
                    }
                }
            }
            ...
            EditorGUI.BeginChangeCheck();
            Vector3 vector13 = MoveHandlesGUI(handleRect, handlePosition, handleRectRotation);
            if (EditorGUI.EndChangeCheck() && !isStatic)
            {
                Vector3 positionDelta = vector13 - TransformManipulator.mouseDownHandlePosition;
