    ...
    public sealed class Handles
    {
        ...
        private static Vector3 DoPlanarHandle(PlaneHandle planeID, Vector3 position, Quaternion rotation, float handleSize)
        {
            ...
            if (Camera.current != null)
            {
                ...
                {
                    normalized = matrixx.inverse.MultiplyVector((Vector3) (Camera.current.transform.rotation * -Vector3.forward)).normalized;
                }
                else
                {
                    normalized = matrixx.inverse.MultiplyPoint(Camera.current.transform.position).normalized;
                }
        ...
        internal static Vector3[] GetBoneVertices(Vector3 endPoint, Vector3 basePoint, float radius)
        {
            ...
            vector.Normalize();
            Vector3 vector3 = Vector3.Cross(lhs, vector);
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                Vector3 vector4 = (Vector3) ((basePoint + (vector * (num3 * radius))) + (vector3 * (num4 * radius)));
                Vector3 vector5 = (Vector3) ((basePoint + (vector * (num5 * radius))) + (vector3 * (num6 * radius)));
