namespace UnityEditor.VersionControl
{
    ...
    {
        [CompilerGenerated]
        private static Predicate<ChangeSet> <>f__am$cache0;
        ...
        {
            <GetEmptyChangeSetsCandidates>c__AnonStorey1 storey = new <GetEmptyChangeSetsCandidates>c__AnonStorey1();
            ChangeSets emptyChangeSets = this.pendingList.EmptyChangeSets;
            storey.toDelete = new ChangeSets();
            if (<>f__am$cache0 == null)
            {
                <>f__am$cache0 = item => item.id != ChangeSet.defaultID;
            }
            emptyChangeSets.FindAll(<>f__am$cache0).ForEach(new Action<ChangeSet>(storey.<>m__0));
        ...
        private void OnGUI()
        {
            ...
            GUIContent content = GUIContent.Temp("Incoming" + ((num2 != 0) ? (" (" + num2.ToString() + ")") : ""));
        ...
        private static void OnVCTaskCompletedEvent(Task task, CompletionAction completionAction)
        {
            ...
            if (completionAction != CompletionAction.OnSubmittedChangeWindow)
            {
                ...
                else if ((completionAction == CompletionAction.OnCheckoutCompleted) && EditorUserSettings.showFailedCheckout)
                {
        ...
        private void PopulateListControl(ListControl list, Task task, Texture2D icon)
        {
            ...
            {
                <PopulateListControl>c__AnonStorey0 storey = new <PopulateListControl>c__AnonStorey0 {
                    cs = firstChild.Item as ChangeSet
                };
                if (changeSets.Find(new Predicate<ChangeSet>(storey.<>m__0)) == null)
                {
        ...
        [CompilerGenerated]
        private sealed class <GetEmptyChangeSetsCandidates>c__AnonStorey1
        {
            internal ChangeSets toDelete;

            internal void <>m__0(ChangeSet s)
            {
                this.toDelete.Add(s);
            }
        }
        ...
        [CompilerGenerated]
        private sealed class <PopulateListControl>c__AnonStorey0
        {
