    ...
    internal class LayoutDropdownWindow : PopupWindowContent
    {
        ...
        internal static unsafe void DrawLayoutMode(Rect position, LayoutMode hMode, LayoutMode vMode, bool doPivot, bool doPosition)
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                switch (((i != 0) ? vMode : hMode))
                {
                    case LayoutMode.Min:
                        ...
                        vector16[i] = rect.min[i] + 0.5f;
                        break;

                    case LayoutMode.Middle:
                        zero[i] = rect.center[i];
                        vector16[i] = rect.center[i];
                        break;

                    case LayoutMode.Max:
                        zero[i] = rect.max[i] - 0.5f;
                        vector16[i] = rect.max[i] - 0.5f;
                        break;
            ...
            if ((doPivot && (hMode != LayoutMode.Undefined)) && (vMode != LayoutMode.Undefined))
            {
                ...
                Vector2 vector27 = new Vector2(x, Mathf.Lerp(rect2.yMin + 0.5f, rect2.yMax - 0.5f, kPivotsForModes[(int) vMode]));
                GUI.color = s_Styles.pivotColor * color;
                GUI.DrawTexture(new Rect(vector27.x - 2.5f, vector27.y - 1.5f, 5f, 3f), EditorGUIUtility.whiteTexture);
                GUI.DrawTexture(new Rect(vector27.x - 1.5f, vector27.y - 2.5f, 3f, 5f), EditorGUIUtility.whiteTexture);
            }
