    ...
    internal class AudioSourceInspector : Editor
    {
        ...
        private void Audio3DGUI()
        {
            ...
            if (this.m_RolloffMode.hasMultipleDifferentValues || ((this.m_RolloffMode.enumValueIndex == 2) && this.m_AudioCurves[0].curveProp.hasMultipleDifferentValues))
            {
                ...
                {
                    <>f__mg$cache0 = new TargetChoiceHandler.TargetChoiceMenuFunction(AudioSourceInspector.SetRolloffToTarget);
                }
                EditorGUILayout.TargetChoiceField(this.m_AudioCurves[0].curveProp, ms_Styles.rolloffLabel, <>f__mg$cache0, new GUILayoutOption[0]);
            }
            else
            {
            ...
            {
                if ((this.m_CurveEditor.GetCurveWrapperFromID(wrapper.id) != null) && this.m_CurveEditor.GetCurveWrapperFromID(wrapper.id).changed)
                {
                    AnimationCurve curve = this.m_CurveEditor.GetCurveWrapperFromID(wrapper.id).curve;
                    if (curve.length > 0)
                    {
                        wrapper.curveProp.animationCurveValue = curve;
                        this.m_CurveEditor.GetCurveWrapperFromID(wrapper.id).changed = false;
        ...
        public override void OnInspectorGUI()
        {
            ...
            foreach (AudioCurveWrapper wrapper in this.m_AudioCurves)
            {
                CurveWrapper curveWrapperFromID = this.m_CurveEditor.GetCurveWrapperFromID(wrapper.id);
                if (wrapper.curveProp != null)
                {
                    AnimationCurve animationCurveValue = wrapper.curveProp.animationCurveValue;
                    if ((curveWrapperFromID == null) != wrapper.curveProp.hasMultipleDifferentValues)
                    {
                        this.m_RefreshCurveEditor = true;
                    }
            ...
            {
                EditorGUILayout.PropertyField(this.m_Spatialize, ms_Styles.spatializeLabel, new GUILayoutOption[0]);
                using (new EditorGUI.DisabledScope(!this.m_Spatialize.boolValue))
                {
                    EditorGUILayout.PropertyField(this.m_SpatializePostEffects, ms_Styles.spatializePostEffectsLabel, new GUILayoutOption[0]);
                }
            }
        ...
        private void Update()
        {
            ...
            Vector3 vector3 = this.m_LastSourcePosition - sourcePos;
            if (vector3.sqrMagnitude <= 0.0001f)
            {
                Vector3 vector4 = this.m_LastListenerPosition - listenerPos;
