namespace UnityEditor
{
    ...
    {
        [CompilerGenerated]
        private static Func<AudioMixerGroupController, bool> <>f__am$cache0;
        [CompilerGenerated]
        private static Func<AudioMixerGroupController, bool> <>f__am$cache1;
        [CompilerGenerated]
        private static Func<AudioMixerGroupController, bool> <>f__am$cache2;
        [CompilerGenerated]
        private static Func<AudioMixerGroupController, bool> <>f__am$cache3;
        [CompilerGenerated]
        private static Func<AudioMixerGroupController, bool> <>f__am$cache4;
        [CompilerGenerated]
        private static Func<AudioMixerEffectController, bool> <>f__am$cache5;
        [CompilerGenerated]
        ...
        internal static void AddEffectItemsToMenu(AudioMixerController controller, AudioMixerGroupController[] groups, int insertIndex, string prefix, GenericMenu pm)
        {
            ...
            for (int i = 0; i < effectList.Length; i++)
            {
                if (effectList[i] != "Attenuation")
                {
                    ...
                    {
                        <>f__mg$cache0 = new GenericMenu.MenuFunction2(AudioMixerChannelStripView.InsertEffectPopupCallback);
                    }
                    pm.AddItem(new GUIContent(prefix + AudioMixerController.FixNameForPopupMenu(effectList[i])), false, <>f__mg$cache0, new EffectContext(controller, groups, insertIndex, effectList[i]));
                }
            }
        }
                    ...
                    if (MixerEffectDefinitions.EffectCanBeSidechainTarget(controller2))
                    {
                        ...
                        if (!AudioMixerController.WillChangeOfEffectTargetCauseFeedback(allGroups, group, effectIndex, controller2, identifiedLoop))
                        {
                            if (showCurrent || (effect.sendTarget != controller2))
                            {
                                ...
                                {
                                    <>f__mg$cache2 = new GenericMenu.MenuFunction2(AudioMixerChannelStripView.ConnectSendPopupCallback);
                                }
                                pm.AddItem(new GUIContent(prefix + "'" + controller2.GetDisplayString(effectMap) + "'"), effect.sendTarget == controller2, <>f__mg$cache2, new ConnectSendContext(effect, controller2));
                            }
                        }
                        else
                        {
        ...
        private void DoEffectSlotInsertEffectPopup(Rect buttonRect, AudioMixerGroupController group, List<AudioMixerGroupController> allGroups, int effectSlotIndex, ref Dictionary<AudioMixerEffectController, AudioMixerGroupController> effectMap)
        {
            ...
            {
                <DoEffectSlotInsertEffectPopup>c__AnonStorey1 storey = new <DoEffectSlotInsertEffectPopup>c__AnonStorey1 {
                    $this = this,
                ...
                {
                    pm.AddItem(new GUIContent("Allow Wet Mixing (causes higher memory usage)"), storey.effect.enableWetMix, new GenericMenu.MenuFunction(storey.<>m__0));
                    pm.AddItem(new GUIContent("Bypass"), storey.effect.bypass, new GenericMenu.MenuFunction(storey.<>m__1));
                    pm.AddSeparator("");
                }
            ...
            if (effectSlotIndex < group.effects.Length)
            {
                ...
                {
                    pm.AddSeparator("");
                    pm.AddItem(new GUIContent("Remove"), false, new GenericMenu.MenuFunction2(this.RemoveEffectPopupCallback), new EffectContext(this.m_Controller, groups, effectSlotIndex, ""));
                    bool flag = false;
                    if (sendEffect.IsSend())
                    {
                        if (sendEffect.sendTarget != null)
                        {
                            ...
                            {
                                <>f__mg$cache1 = new GenericMenu.MenuFunction2(AudioMixerChannelStripView.ConnectSendPopupCallback);
                            }
                            pm.AddItem(new GUIContent("Disconnect from '" + sendEffect.GetSendTargetDisplayString(effectMap) + "'"), false, <>f__mg$cache1, new ConnectSendContext(sendEffect, null));
                        }
                        if (!flag)
                        {
        ...
        private bool DoMuteButton(Rect r, AudioMixerGroupController group, List<AudioMixerGroupController> allGroups, bool anySoloActive, List<AudioMixerGroupController> selection)
        {
            ...
            bool flag2 = anySoloActive && group.mute;
            if (flag2)
            {
                GUI.color = new Color(GUI.color.r, GUI.color.g, GUI.color.b, 0.5f);
            }
            bool flag3 = GUI.Toggle(r, group.mute, this.styles.muteGUIContent, AudioMixerDrawUtils.styles.muteToggle);
            if (flag2)
            {
                GUI.color = color;
            }
        ...
        private float DoVUMeters(Rect vuRect, float attenuationMarkerHeight, ChannelStripParams p)
        {
            ...
            else if ((p.index >= 0) && (p.index < this.m_LastNumChannels.Count))
            {
        ...
        private void EffectSlot(Rect effectRect, AudioMixerSnapshotController snapshot, AudioMixerEffectController effect, int effectIndex, ref int highlightEffectIndex, ChannelStripParams p, ref Dictionary<AudioMixerEffectController, PatchSlot> patchslots)
        {
            if (effect != null)
            {
                ...
                if (current.type == EventType.Repaint)
                {
                    ...
                    float num2 = !hasKeyboardFocus ? 1f : ((level - AudioMixerController.kMinVolume) / (AudioMixerController.kMaxEffect - AudioMixerController.kMinVolume));
        ...
        private void EffectSlotDragging(Rect r, AudioMixerSnapshotController snapshot, AudioMixerEffectController effect, bool showLevel, float level, int effectIndex, ref int highlightEffectIndex, ChannelStripParams p)
        {
            ...
            else if ((typeForControl == EventType.MouseDrag) && (GUIUtility.hotControl == this.m_EffectInteractionControlID))
            {
        ...
        {
            <GroupClicked>c__AnonStorey0 storey = new <GroupClicked>c__AnonStorey0();
        ...
        private static bool RectOverlaps(Rect r1, Rect r2)
        {
            ...
            rect.width = Mathf.Min((float) (r1.x + r1.width), (float) (r2.x + r2.width)) - rect.x;
            rect.height = Mathf.Min((float) (r1.y + r1.height), (float) (r2.y + r2.height)) - rect.y;
        ...
        {
            internal AudioMixerChannelStripView $this;
            ...
            internal void <>m__1()
            {
                ...
                this.$this.m_Controller.UpdateBypass();
                InspectorWindow.RepaintAllInspectors();
            }
        }

        [CompilerGenerated]
        private sealed class <GroupClicked>c__AnonStorey0
        {
