namespace UnityEditor
{
    ...
    internal class FlexibleMenu : PopupWindowContent
    {
        ...
        private void CreateNewItemButton(Rect itemRect)
        {
            if (this.m_ModifyItemUI != null)
            {
                ...
                if (GUI.Button(position, s_Styles.plusButtonText, "OL Plus"))
                {
                    ...
                    PopupWindow.Show(position, this.m_ModifyItemUI, null, ShowMode.PopupMenuWithKeyboardFocus);
                }
            }
        }
        ...
        {
            <EditExistingItem>c__AnonStorey0 storey = new <EditExistingItem>c__AnonStorey0 {
                index = index,
                $this = this
            };
            if (this.m_ModifyItemUI != null)
            {
                ...
                this.m_ModifyItemUI.Init(FlexibleMenuModifyItemUI.MenuType.Edit, this.m_ItemProvider.GetItem(storey.index), new Action<object>(storey.<>m__0));
        ...
        [CompilerGenerated]
        private sealed class <EditExistingItem>c__AnonStorey0
        {
            internal FlexibleMenu $this;
            internal int index;

            internal void <>m__0(object obj)
            {
                this.$this.ClearCachedWidth();
                this.$this.m_ItemProvider.Replace(this.index, obj);
                EditorApplication.RequestRepaintAllViews();
            }
        }
        ...
        internal static class ItemContextMenu
        {
            ...
            public static void Show(int itemIndex, FlexibleMenu caller)
            {
                ...
                {
                    <>f__mg$cache0 = new GenericMenu.MenuFunction2(FlexibleMenu.ItemContextMenu.Edit);
                }
                menu.AddItem(new GUIContent("Edit..."), false, <>f__mg$cache0, itemIndex);
                if (<>f__mg$cache1 == null)
                {
                    <>f__mg$cache1 = new GenericMenu.MenuFunction2(FlexibleMenu.ItemContextMenu.Delete);
                }
                menu.AddItem(new GUIContent("Delete"), false, <>f__mg$cache1, itemIndex);
                menu.ShowAsContext();
                GUIUtility.ExitGUI();
            }
        }
