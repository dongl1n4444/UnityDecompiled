namespace UnityEditor
{
    ...
    {
        [CompilerGenerated]
        private static Func<PopupList.ListElement, bool> <>f__am$cache0;
        [CompilerGenerated]
        private static Func<PopupList.ListElement, string> <>f__am$cache1;
        [CompilerGenerated]
        private static Func<PopupList.ListElement, bool> <>f__am$cache2;
        [CompilerGenerated]
        private static Func<PopupList.ListElement, string> <>f__am$cache3;
        ...
        private void EnsureValidSetup()
        {
            ...
            if ((this.m_ViewMode == ViewMode.TwoColumns) && (this.m_SearchFilter.GetState() == SearchFilter.State.EmptySearchFilter))
            {
                this.SelectAssetsFolder();
            }
        }
        ...
        public string[] GetCurrentVisibleNames() => 
            this.m_ListArea.GetCurrentVisibleNames();
                    ...
                    default:
                        switch (keyCode)
                        {
                            case KeyCode.Backspace:
                                if ((Application.platform != RuntimePlatform.OSXEditor) && (this.m_ViewMode == ViewMode.TwoColumns))
                                {
                                    this.ShowParentFolderOfCurrentlySelected();
                                    Event.current.Use();
                                }
                            ...
                            case KeyCode.F2:
                                if ((Application.platform != RuntimePlatform.OSXEditor) && this.m_ListArea.BeginRename(0f))
                                {
                                    Event.current.Use();
                                }
                                return;
        ...
        private void OnGUI()
        {
            ...
            else
            {
                ...
                if ((this.m_SearchFilter.GetState() == SearchFilter.State.FolderBrowsing) && (this.m_ListArea.numItemsDisplayed == 0))
                {
                    ...
                    Rect rect6 = new Rect((this.m_ListAreaRect.x + 2f) + Mathf.Max((float) 0f, (float) ((this.m_ListAreaRect.width - vector.x) * 0.5f)), this.m_ListAreaRect.y + 10f, vector.x, 20f);
                    using (new EditorGUI.DisabledScope(true))
                    {
                        GUI.Label(rect6, s_Styles.m_EmptyFolderText, EditorStyles.label);
                    }
                }
            }
        ...
        private void SearchButtonClickedCallback(ExposablePopupMenu.ItemData itemClicked)
        {
            if (!itemClicked.m_On)
            {
                this.SetSearchViewState((SearchViewState) itemClicked.m_UserData);
        ...
        private void ShowParentFolderOfCurrentlySelected()
        {
            if (this.IsShowingFolderContents())
            {
                ...
                if (selection.Length == 1)
                {
                    TreeViewItem item = this.m_FolderTree.FindItem(selection[0]);
