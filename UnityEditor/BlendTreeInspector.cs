namespace UnityEditor
{
    ...
    {
        [CompilerGenerated]
        private static Func<Vector2, bool> <>f__am$cache0;
        [CompilerGenerated]
        private static Func<Vector2, float> <>f__am$cache1;
        [CompilerGenerated]
        private static Func<float, float> <>f__am$cache2;
        [CompilerGenerated]
        private static GetFloatFromMotion <>f__am$cache3;
        [CompilerGenerated]
        private static GetFloatFromMotion <>f__am$cache4;
        [CompilerGenerated]
        private static GetFloatFromMotion <>f__am$cache5;
        [CompilerGenerated]
        private static GetFloatFromMotion <>f__am$cache6;
        [CompilerGenerated]
        ...
        public float ClickDragFloat(Rect position, float value, bool alignRight)
        {
            ...
            switch (current.type)
            {
                ...
                case EventType.MouseDown:
                    if (((GUIUtility.keyboardControl != id) || !EditorGUIUtility.editingTextField) && position.Contains(current.mousePosition))
                    {
                        current.Use();
                        s_ClickDragFloatDragged = false;
        ...
        private void ComputeFromAngularSpeedDegrees(object obj)
        {
            ...
            {
                <>f__am$cache7 = (m, mirrorMultiplier) => ((m.averageAngularSpeed * 180f) / 3.141593f) * mirrorMultiplier;
            }
            this.ComputeProperty(<>f__am$cache7, prop);
        }
        ...
        private void ComputeFromAngularSpeedRadians(object obj)
        {
            ...
            {
                <>f__am$cache8 = (m, mirrorMultiplier) => m.averageAngularSpeed * mirrorMultiplier;
            }
            this.ComputeProperty(<>f__am$cache8, prop);
        }
        ...
        private void ComputeFromSpeed(object obj)
        {
            ...
            {
                <>f__am$cache3 = (m, mirrorMultiplier) => m.apparentSpeed;
            }
            this.ComputeProperty(<>f__am$cache3, prop);
        }
        ...
        private void ComputeFromVelocityX(object obj)
        {
            ...
            {
                <>f__am$cache4 = (m, mirrorMultiplier) => m.averageSpeed.x * mirrorMultiplier;
            }
            this.ComputeProperty(<>f__am$cache4, prop);
        }
        ...
        private void ComputeFromVelocityY(object obj)
        {
            ...
            {
                <>f__am$cache5 = (m, mirrorMultiplier) => m.averageSpeed.y;
            }
            this.ComputeProperty(<>f__am$cache5, prop);
        }
        ...
        private void ComputeFromVelocityZ(object obj)
        {
            ...
            {
                <>f__am$cache6 = (m, mirrorMultiplier) => m.averageSpeed.z;
            }
            this.ComputeProperty(<>f__am$cache6, prop);
        }
        ...
        public void DrawChild(Rect r, int index, bool isActive, bool isFocused)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < 2; i++)
                {
                    ...
                    num++;
                    if (EditorGUI.EndChangeCheck() && float.TryParse(str3, NumberStyles.Float, (IFormatProvider) CultureInfo.InvariantCulture.NumberFormat, out num5))
                    {
                        vector[i] = Mathf.Clamp(num5, -10000f, 10000f);
                        property4.vector2Value = vector;
                        base.serializedObject.ApplyModifiedProperties();
                        GUI.changed = true;
                    }
            ...
            if (EditorGUILayout.BeginFadeGroup(this.m_ShowGraph.faded))
            {
                ...
                else if (this.m_BlendType.intValue == 4)
                {
                    for (int i = 0; i < this.m_BlendTree.recursiveBlendParameterCount; i++)
                    {
                        ...
                        EditorGUI.BeginChangeCheck();
                        float parameterValue = EditorGUILayout.Slider(recursiveBlendParameter, GetParameterValue(currentAnimator, this.m_BlendTree, recursiveBlendParameter), recursiveBlendParameterMin, recursiveBlendParameterMax, new GUILayoutOption[0]);
                        if (EditorGUI.EndChangeCheck())
                        {
                            SetParameterValue(currentAnimator, this.m_BlendTree, parentBlendTree, recursiveBlendParameter, parameterValue);
                        }
                    }
                }
                else
