namespace UnityEditor
{
    ...
    internal class AudioMixerWindow : EditorWindow, IHasCustomMenu
    {
        ...
        [SerializeField]
        private LayoutMode m_LayoutMode = LayoutMode.Vertical;
        [SerializeField]
        ...
        public virtual void AddItemsToMenu(GenericMenu menu)
        {
            ...
            menu.AddItem(new GUIContent("Use RMS metering for display"), EditorPrefs.GetBool(kAudioMixerUseRMSMetering, true), <>f__am$cache0);
            if (<>f__am$cache1 == null)
            {
                <>f__am$cache1 = () => EditorPrefs.SetBool(kAudioMixerUseRMSMetering, false);
            }
            menu.AddItem(new GUIContent("Use peak metering for display"), !EditorPrefs.GetBool(kAudioMixerUseRMSMetering, true), <>f__am$cache1);
            if (Unsupported.IsDeveloperBuild())
            {
                menu.AddSeparator("");
                menu.AddItem(new GUIContent("DEVELOPER/Groups Rendered Above"), this.m_GroupsRenderedAboveSections, () => this.m_GroupsRenderedAboveSections = !this.m_GroupsRenderedAboveSections);
                ...
                menu.AddItem(new GUIContent("DEVELOPER/Show overlays"), this.m_ShowDeveloperOverlays, () => this.m_ShowDeveloperOverlays = !this.m_ShowDeveloperOverlays);
            }
        }
        ...
        private void LayoutWithStripsOnRightSideOneScrollBar(List<AudioMixerGroupController> allGroups, Dictionary<AudioMixerEffectController, AudioMixerGroupController> effectMap)
        {
            ...
            Rect rect4 = new Rect(width, 17f, num2, rect.height);
            ...
            GUI.EndScrollView();
            this.m_ChannelStripView.OnGUI(rect4, this.m_ShowReferencedBuses, this.m_ShowBusConnections, this.m_ShowBusConnectionsOfSelection, allGroups, effectMap, this.m_SortGroupsAlphabetically, this.m_ShowDeveloperOverlays, this.m_GroupTree.ScrollToItem);
        ...
        private void LayoutWithStripsOnTop(List<AudioMixerGroupController> allGroups, Dictionary<AudioMixerEffectController, AudioMixerGroupController> effectMap)
        {
            ...
            float y = !this.m_GroupsRenderedAboveSections ? ((float) (0x11 + verticalSplitter.realSizes[0])) : ((float) 0x11);
            float height = !this.m_GroupsRenderedAboveSections ? ((float) verticalSplitter.realSizes[1]) : ((float) verticalSplitter.realSizes[0]);
            float num3 = this.m_GroupsRenderedAboveSections ? ((float) (0x11 + verticalSplitter.realSizes[0])) : ((float) 0x11);
            ...
            EditorGUI.DrawRect(new Rect(0f, (float) ((0x11 + verticalSplitter.realSizes[0]) - 1), base.position.width, 1f), new Color(0f, 0f, 0f, 0.4f));
        }
        ...
        private class GUIContents
        {
            ...
            public GUIContent selectAudioMixer = new GUIContent("", "Select an Audio Mixer");
