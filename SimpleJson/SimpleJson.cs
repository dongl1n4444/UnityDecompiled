    ...
    internal static class SimpleJson
    {
            ...
            while (!flag)
            {
                ...
                if (ch == '\\')
                {
                    ...
                    else if (ch == 'u')
                    {
                        ...
                        if (!(success = uint.TryParse(new string(json, index, 4), NumberStyles.HexNumber, CultureInfo.InvariantCulture, out num3)))
                        {
                            return "";
                        }
                        if ((0xd800 <= num3) && (num3 <= 0xdbff))
                        {
                            uint num4;
                            index += 4;
                            num2 = json.Length - index;
                            if (((num2 < 6) || ((new string(json, index, 2) != @"\u") || !uint.TryParse(new string(json, index + 2, 4), NumberStyles.HexNumber, CultureInfo.InvariantCulture, out num4))) || ((0xdc00 > num4) || (num4 > 0xdfff)))
                            {
                                success = false;
                                return "";
                            }
                        ...
                        builder.Append(ConvertFromUtf32((int) num3));
                        index += 4;
                    }
                }
                else
        ...
        public static IJsonSerializerStrategy CurrentJsonSerializerStrategy
        {
            get
            {
                ...
                {
                    strategy = _currentJsonSerializerStrategy;
                }
                else
                {
                    strategy = _currentJsonSerializerStrategy = PocoJsonSerializerStrategy;
                }
                return strategy;
            }
            set
        ...
        public static SimpleJson.PocoJsonSerializerStrategy PocoJsonSerializerStrategy
        {
            get
            {
                SimpleJson.PocoJsonSerializerStrategy strategy;
                if (_pocoJsonSerializerStrategy != null)
                {
                    strategy = _pocoJsonSerializerStrategy;
                }
                else
                {
                    strategy = _pocoJsonSerializerStrategy = new SimpleJson.PocoJsonSerializerStrategy();
                }
                return strategy;
            }
        }
