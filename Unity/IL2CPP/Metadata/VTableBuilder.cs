namespace Unity.IL2CPP.Metadata
{
    ...
    public class VTableBuilder
    {
            ...
            foreach (KeyValuePair<MethodReference, MethodDefinition> pair in overrides)
            {
                ...
                if (!declaringType.Resolve().IsInterface)
                {
                    ...
                    if (reference3 != null)
                    {
                        ...
                        {
                            throw new InvalidOperationException($"Error while creating VTable for {declaringType}. The base method {key} is implemented both by {reference3} and {method}.");
                        }
                    }
                    else
                    {
        ...
        private void SetupInterfaceMethods(TypeDefinition typeDefinition, Dictionary<TypeReference, int> interfaceOffsets, Dictionary<MethodReference, MethodReference> overrideMap, List<MethodReference> slots)
        {
            foreach (KeyValuePair<TypeReference, int> pair in interfaceOffsets)
            {
                ...
                foreach (MethodReference reference in definition.Methods.Where<MethodDefinition>(<>f__am$cache3).Select<MethodDefinition, MethodReference>(new Func<MethodDefinition, MethodReference>(storey.<>m__0)))
                {
                    ...
                    {
                        foreach (MethodReference reference3 in typeDefinition.GetVirtualMethods())
                        {
            ...
            if (!typeDefinition.IsAbstract)
            {
                foreach (KeyValuePair<TypeReference, int> pair in interfaceOffsets)
                {
                    ...
                    foreach (MethodReference reference2 in key.Resolve().Methods.Where<MethodDefinition>(<>f__am$cache4).Select<MethodDefinition, MethodReference>(new Func<MethodDefinition, MethodReference>(resolver.Resolve)))
                    {
                        ...
                        if (slots[num2] == null)
                        {
                            throw new Exception($"Interface {key.FullName} method {reference2.FullName} not implemented on non-abstract class {typeDefinition.FullName}");
                        }
                    }
                }
            }
        ...
        public VTable VTableFor(TypeReference typeReference, Unity.IL2CPP.ILPreProcessor.TypeResolver resolver = null)
        {
            VTable table;
        ...
        private sealed class <SetupInterfaceMethods>c__AnonStorey0
        {
            ...
            internal MethodReference <>m__0(MethodDefinition m) => 
                Unity.IL2CPP.ILPreProcessor.TypeResolver.For(this.itf).Resolve(m);
        }
    }
}
