namespace Unity.IL2CPP
{
    ...
    internal class InterfaceInvokerCollector
    {
        ...
        private static string InvokeParametersFor(InvokerData data, bool isGeneric)
        {
            ...
            {
                strArray = !isGeneric ? new string[] { "MonoMethod* slot", $"{"RuntimeClass"}* declaringInterface", "void* obj" } : new string[] { "MonoMethod* method", "void* obj" };
            }
            else
            {
                strArray = !isGeneric ? new string[] { "Il2CppMethodSlot slot", $"{"RuntimeClass"}* declaringInterface", "Il2CppObject* obj" } : new string[] { "const MethodInfo* method", "Il2CppObject* obj" };
            }
        ...
        public void Process(AssemblyDefinition assembly)
        {
            foreach (TypeDefinition definition in assembly.MainModule.GetAllTypes())
            {
                ...
                {
                    if (definition2.DeclaringType.IsInterface && (this._processGenerics == definition2.HasGenericParameters))
                    {
            ...
            using (SourceCodeWriter writer = new SourceCodeWriter(path))
            {
                ...
                foreach (InvokerData data in this._invokerData.OrderBy<InvokerData, int>(<>f__am$cache0))
                {
                    ...
                    else
                    {
                        ...
                        writer.WriteLine("struct Interface{0}Invoker{1}", objArray3);
                    }
                    writer.BeginBlock();
