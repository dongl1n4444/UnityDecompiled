...
namespace Unity.IL2CPP.Marshaling
{
	public struct ManagedMarshalValue
	{
		...
		public ManagedMarshalValue Dereferenced
		{
			get
			{
				ManagedMarshalValue result;
				if (this._objectVariableName.StartsWith("&"))
				{
					result = new ManagedMarshalValue(this._objectVariableName.Substring(1), this._field, this._indexVariableName);
				}
				else
				{
					result = new ManagedMarshalValue("*" + this._objectVariableName, this._field, this._indexVariableName);
				}
				return result;
			}
		}
		...
		public string Load()
		{
			...
			if (this._indexVariableName != null)
			{
				...
				result = Emit.LoadArrayElement(array, this._indexVariableName, false);
			}
		...
		public string Store(string value)
		{
			...
			if (this._indexVariableName != null)
			{
				...
				result = string.Format("{0};", Emit.StoreArrayElement(array, this._indexVariableName, value, false));
			}
			else if (this._field != null)
			{
