	...
	public class MetadataCollector : IMetadataCollection
	{
		...
		public void AddTypeInfos(IEnumerable<TypeDefinition> types)
		{
			MetadataCollector.AddUnique<TypeDefinition>(this._typeInfos, types, delegate(TypeDefinition type)
			{
				...
				this.AddMethods(from m in type.Methods
				where !m.IsStripped()
				select m);
				...
				if (type.HasInterfaces)
				{
					...
					this._interfaces.AddRange(type.Interfaces.Select((InterfaceImplementation a, int b) => MetadataCollector.Il2CppTypeCollector.GetOrCreateIndex(a.InterfaceType, b)));
				}
				if (type.HasGenericParameters)
				{
					...
					if (runtimeGenericDatas.Count > 0)
					{
						...
						List<KeyValuePair<int, uint>> arg_20C_0 = this._rgctxEntries;
						IEnumerable<RuntimeGenericData> arg_207_0 = runtimeGenericDatas;
						...
						arg_20C_0.AddRange(arg_207_0.Select(MetadataCollector.<>f__mg$cache0));
					}
				}
			});
		}
		...
		private void AddReferencedAssemblyMetadata(ICollection<AssemblyDefinition> assemblies)
		{
			foreach (AssemblyDefinition current in assemblies)
			{
				List<AssemblyDefinition> list = MetadataCollector.AssemblyDependencies.GetReferencedAssembliesFor(current).ToList<AssemblyDefinition>();
				if (list.Count == 0)
				{
