	...
	public class MetadataUtils
	{
		...
		internal static byte[] ConstantDataFor(IConstantProvider constantProvider, TypeReference declaredParameterOrFieldType, string name)
		{
			...
			if (genericInstanceType == null || !TypeReferenceEqualityComparer.AreEqual(declaredParameterOrFieldType.Resolve(), MetadataUtils.TypeProvider.SystemNullable, TypeComparisonMode.Exact))
			{
				...
				{
					obj = MetadataUtils.ChangePrimitiveType(obj, declaredParameterOrFieldType);
				}
		...
		public static object ChangePrimitiveType(object o, TypeReference type)
		{
			...
			else if (o is int && type.MetadataType == MetadataType.UInt32)
			{
				result = (uint)((int)o);
			}
			else
			{
				result = Convert.ChangeType(o, MetadataUtils.DetermineTypeForDefaultValueBasedOnDeclaredType(type, o));
			}
			return result;
		}
