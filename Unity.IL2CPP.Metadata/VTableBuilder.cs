...
namespace Unity.IL2CPP.Metadata
{
	public class VTableBuilder
	{
		...
		private void SetupMethodSlotsForInterface(TypeReference typeReference)
		{
			...
			where m.IsVirtual && !m.IsStatic && !m.IsStripped()
			select m)
			{
		...
		{
			foreach (MethodDefinition current in from m in typeDefinition.Methods
			where m.IsVirtual && !m.IsStripped()
			select m)
			{
		...
		{
			return typeDefinition.BaseType == null || typeDefinition.Interfaces.Any((InterfaceImplementation classItf) => TypeReferenceEqualityComparer.AreEqual(itf, classItf.InterfaceType, TypeComparisonMode.Exact));
		}
