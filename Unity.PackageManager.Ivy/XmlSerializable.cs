...
namespace Unity.PackageManager.Ivy
{
	public class XmlSerializable : IXmlSerializable
	{
			...
			foreach (KeyValuePair<string, XmlAttributeAttribute> current in dictionary)
			{
				if (XmlSerializable.attributeMappings.ContainsKey(current.Key))
				{
					...
					if (obj2 != null)
					{
						...
						object obj3 = null;
						if (dictionary3.TryGetValue(current.Key, out obj3))
						{
							if (obj2 == obj3)
							{
								continue;
						...
						if (obj2 is IvyPackageType && this is Unity.PackageManager.IvyInternal.IvyInfo)
						{
							...
							{
								IvyPackageType ivyPackageType = (IvyPackageType)obj2;
						...
						writer.WriteAttributeString((current.Value.Namespace == null) ? "" : "e", current.Value.AttributeName, current.Value.Namespace, text);
					}
				}
			}
		...
		{
			object obj = XmlSerializable.typeMappings;
			lock (obj)
			{
		...
		{
			Type type = base.GetType();
			object obj = XmlSerializable.processed;
			lock (obj)
			{
