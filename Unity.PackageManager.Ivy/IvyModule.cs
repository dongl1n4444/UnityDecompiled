...
namespace Unity.PackageManager.Ivy
{
	public class IvyModule
	{
		...
		public override bool Equals(object other)
		{
			...
			{
				result = (this == other as PackageInfo);
			}
			else
			{
				result = (this == other as IvyModule);
			}
			return result;
		}
		...
		public PackageInfo ToPackageInfo()
		{
			...
			packageInfo.files = this.Artifacts.ToDictionary((IvyArtifact p) => p.Filename, (IvyArtifact e) => new PackageFileData((PackageFileType)e.Type, (!(e.Url != null)) ? "" : e.Url.ToString(), e.Guid ?? ""));
