	...
	internal class AnimatorControllerTool : EditorWindow, IAnimatorControllerEditor
	{
		...
		private class BreadCrumbElement
		{
			...
			public string name
			{
				get
				{
					return (!this.m_Target) ? "" : this.m_Target.name;
				}
			}
		...
		protected GraphGUI activeGraphGUI
		{
			get
			{
				...
				else if (this.m_BreadCrumbs.Last<AnimatorControllerTool.BreadCrumbElement>().target is AnimatorStateMachine)
				{
					result = this.stateMachineGraphGUI;
				}
				else
				{
					result = this.blendTreeGraphGUI;
				}
				return result;
			}
		}
		...
		protected IAnimatorControllerSubEditor editor
		{
			get
			{
				...
				IAnimatorControllerSubEditor result;
				if (currentEditor != 1)
				{
					if (currentEditor != 0)
					{
					}
					result = this.m_LayerEditor;
				}
				else
				{
					result = this.m_ParameterEditor;
				}
				return result;
			}
		}
		...
		{
			return controller && controller.name == "" && controller.layers.Length > 0 && controller.layers[0].name == "preview" && controller.hideFlags == HideFlags.DontSave;
		}
		...
		private void DetectAnimatorControllerFromSelection()
		{
			...
			if (Selection.activeGameObject)
			{
				...
				if (component)
				{
					if (!AnimatorController.FindAnimatorControllerPlayable(component, this.animatorController).IsValid())
					{
		...
		private void DetectPreviewObjectFromSelection()
		{
			if (Selection.activeGameObject)
			{
				...
				if (component)
				{
					if (AnimatorController.GetEffectiveAnimatorController(component) != null || component.runtimeAnimatorController != null)
					{
		...
		private AnimatorStateMachine GetParentStateMachine()
		{
			...
			{
				result = (this.m_BreadCrumbs[this.m_BreadCrumbs.Count - 2].target as AnimatorStateMachine);
			}
			return result;
		}
