...
namespace UnityEditor.Graphs
{
	...
	internal class LayerControllerView : IAnimatorControllerSubEditor
	{
		...
		private void OnReorderLayer(ReorderableList reorderablelist)
		{
			...
			this.RemapIndices(layers, array);
		...
		private void OnDrawLayer(Rect rect, int index, bool selected, bool focused)
		{
			...
			if (animatorControllerLayer.avatarMask != null)
			{
				...
				if (index == 0)
				{
					if (this.IsLayerUsingHumanoid(animatorControllerLayer))
					{
						Vector2 vector6 = LayerControllerView.s_Styles.invisibleButton.CalcSize(LayerControllerView.s_Styles.maskWarningIcon);
						float num = (rect3.height - vector6.y) * 0.5f;
						rect3 = new Rect(rect3.xMin - vector6.x - 4f, rect.yMin + num, vector6.x, vector6.y);
						GUI.Label(rect3, LayerControllerView.s_Styles.maskWarningIcon, LayerControllerView.s_Styles.invisibleButton);
					}
				}
			}
			Rect rect4 = Rect.MinMaxRect(rect.xMin, rect.yMin, rect3.xMin - 4f, rect.yMax - 16f);
			float num2 = (float)LayerControllerView.s_Styles.label.padding.left;
			Rect rect5 = Rect.MinMaxRect(rect.xMin + num2, rect.yMax - 11f, rect2.xMax, rect.yMax - 9f);
			...
			{
				float num3 = (index != 0) ? ((!this.m_Host.liveLink) ? animatorControllerLayer.defaultWeight : this.m_Host.previewAnimator.GetLayerWeight(index)) : 1f;
				Rect rect6 = rect5;
				rect6.width *= num3;
		...
		private bool IsLayerUsingHumanoid(UnityEditor.Animations.AnimatorControllerLayer layer)
		{
			...
			for (int i = 0; i < states.Length; i++)
			{
				...
				else if (state.motion is UnityEditor.Animations.BlendTree)
				{
					if (this.IsBlendTreeUsingHumanoid(state.motion as UnityEditor.Animations.BlendTree))
					{
						result = true;
						return result;
					}
		...
		private bool IsBlendTreeUsingHumanoid(UnityEditor.Animations.BlendTree blendTree)
		{
			...
			for (int i = 0; i < children.Length; i++)
			{
				...
				else if (motion is UnityEditor.Animations.BlendTree)
				{
					if (this.IsBlendTreeUsingHumanoid(motion as UnityEditor.Animations.BlendTree))
					{
						result = true;
						return result;
					}
		...
		protected void OnRemoveLayer(int layerIndex)
		{
			...
			if (list.Count > 0)
			{
				...
				else if (list.Count == 1 || EditorUtility.DisplayDialog("Deleting synchronized layer", "Deleting this layer will also delete all the layers that are synchronized on it. Are you sure you want to delete it?", "Delete", "Cancel"))
				{
					...
					UnityEditor.Animations.AnimatorControllerLayer[] layers2 = this.m_Host.animatorController.layers;
					this.RemapIndices(layers, layers2);
					this.m_Host.animatorController.layers = layers2;
					this.ResetUI();
					this.m_Host.ResetUI();
				}
			}
