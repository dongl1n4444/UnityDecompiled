...
namespace UnityEditor.Graphs
{
	...
	public abstract class GraphGUI : ScriptableObject
	{
		...
		private GraphGUI.SelectionDragMode m_IsDraggingSelection = GraphGUI.SelectionDragMode.None;
		...
		private static Color gridMinorColor
		{
			get
			{
				...
				{
					result = GraphGUI.kGridMinorColorDark;
				}
				else
				{
					result = GraphGUI.kGridMinorColorLight;
				}
				return result;
			}
		}
		...
		private static Color gridMajorColor
		{
			get
			{
				...
				if (EditorGUIUtility.isProSkin)
				{
					result = GraphGUI.kGridMajorColorDark;
				}
				else
				{
					result = GraphGUI.kGridMajorColorLight;
				}
				return result;
			}
		}
		...
		private void UpdateGraphExtents()
		{
			...
			Graph expr_1C_cp_0 = this.graph;
			expr_1C_cp_0.graphExtents.xMin = expr_1C_cp_0.graphExtents.xMin - this.m_Host.position.width * 0.6f;
			Graph expr_4C_cp_0 = this.graph;
			expr_4C_cp_0.graphExtents.xMax = expr_4C_cp_0.graphExtents.xMax + this.m_Host.position.width * 0.6f;
			Graph expr_7C_cp_0 = this.graph;
			expr_7C_cp_0.graphExtents.yMin = expr_7C_cp_0.graphExtents.yMin - this.m_Host.position.height * 0.6f;
			Graph expr_AC_cp_0 = this.graph;
			expr_AC_cp_0.graphExtents.yMax = expr_AC_cp_0.graphExtents.yMax + this.m_Host.position.height * 0.6f;
		}
		...
		private void ContextMenuClick(object userData, string[] options, int selected)
		{
			if (selected >= 0)
			{
				...
				if (text != null)
				{
					if (!(text == "Cut") && !(text == "Copy") && !(text == "Duplicate") && !(text == "Delete"))
					{
						if (text == "Paste")
						{
				...
				foreach (Slot current2 in current.slots)
				{
					foreach (Edge current3 in current2.edges)
					{
						...
						else
						{
							...
							Edge key;
							(dictionary2 = dictionary)[key = current3] = dictionary2[key] + 1;
						}
					}
				}
			}
