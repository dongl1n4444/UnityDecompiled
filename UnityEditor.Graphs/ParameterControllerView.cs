...
namespace UnityEditor.Graphs
{
	internal class ParameterControllerView : IAnimatorControllerSubEditor
	{
		...
		private class FloatElement : ParameterControllerView.Element
		{
			public float value
			{
				get
				{
					...
					{
						result = this.m_Host.m_Host.previewAnimator.GetFloat(base.name);
					}
					else
					{
						result = this.m_Parameter.defaultFloat;
					}
					return result;
				}
				set
		...
		private class BoolElement : ParameterControllerView.Element
		{
			public bool value
			{
				get
				{
					...
					if (this.m_Host.m_Host.liveLink)
					{
						result = this.m_Host.m_Host.previewAnimator.GetBool(base.name);
					}
					else
					{
						result = this.m_Parameter.defaultBool;
					}
					return result;
				}
				set
			...
			{
				EditorGUI.BeginChangeCheck();
				bool value = GUI.Toggle(rect, this.value, "");
				if (EditorGUI.EndChangeCheck())
				{
					this.value = value;
				}
		...
		private class IntElement : ParameterControllerView.Element
		{
			public int value
			{
				get
				{
					...
					if (this.m_Host.m_Host.liveLink)
					{
						result = this.m_Host.m_Host.previewAnimator.GetInteger(base.name);
					}
					else
					{
						result = this.m_Parameter.defaultInt;
					}
					return result;
				}
				set
		...
		private class TriggerElement : ParameterControllerView.BoolElement
		{
			...
			{
				EditorGUI.BeginChangeCheck();
				bool value = GUI.Toggle(rect, base.value, "", ParameterControllerView.s_Styles.triggerButton);
				if (EditorGUI.EndChangeCheck())
				{
					base.value = value;
				}
		...
		private ParameterControllerView.Element CreateElement(UnityEngine.AnimatorControllerParameter parameter)
		{
			...
			{
				ParameterControllerView.Element result = new ParameterControllerView.FloatElement(parameter, this);
				return result;
			}
			case (UnityEngine.AnimatorControllerParameterType)2:
			{
				IL_20:
				ParameterControllerView.Element result;
				if (type != UnityEngine.AnimatorControllerParameterType.Trigger)
				{
					result = null;
					return result;
				}
				result = new ParameterControllerView.TriggerElement(parameter, this);
				return result;
			}
			case UnityEngine.AnimatorControllerParameterType.Int:
			{
				ParameterControllerView.Element result = new ParameterControllerView.IntElement(parameter, this);
				return result;
			}
			case UnityEngine.AnimatorControllerParameterType.Bool:
			{
				ParameterControllerView.Element result = new ParameterControllerView.BoolElement(parameter, this);
				return result;
			}
			}
			goto IL_20;
		...
		protected void CreateSearchParameterList(string newSearch, int searchMode)
		{
			...
			while (i < tree.Length)
			{
				...
				ParameterControllerView.SearchMode searchMode2 = (ParameterControllerView.SearchMode)searchMode;
				if (!(a != searchMode2.ToString()))
				{
					goto IL_81;
				}
				IL_121:
