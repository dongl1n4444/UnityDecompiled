    ...
    public class ReorderableList
    {
                ...
                if (this.IsDragging() && (Event.current.type == EventType.Repaint))
                {
                    ...
                    for (int j = 0; j < this.m_NonDragTargetIndices.Count; j++)
                    {
                        if (this.m_NonDragTargetIndices[j] != -1)
                        {
                            ...
                            {
                                r.y = listRect.y + this.GetElementYOffset(this.m_NonDragTargetIndices[j], this.m_ActiveElement);
                                if (flag)
                                {
                                    r.y += this.elementHeightCallback(this.m_ActiveElement);
                                }
                            }
        ...
        private float GetElementYOffset(int index, int skipIndex)
        {
            ...
            for (int i = 0; i < index; i++)
            {
                if (i != skipIndex)
                {
                    num2 += this.elementHeightCallback(i);
                }
            }
            return num2;
        }
        ...
        private int GetRowIndex(float localY)
        {
            ...
            {
                float num4 = this.elementHeightCallback(i);
                float num5 = num2 + num4;
                if ((localY >= num2) && (localY < num5))
                {
                    return i;
                }
                num2 += num4;
