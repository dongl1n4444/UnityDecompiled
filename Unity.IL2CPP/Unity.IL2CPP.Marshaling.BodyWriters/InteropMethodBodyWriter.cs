...
namespace Unity.IL2CPP.Marshaling.BodyWriters
{
	internal abstract class InteropMethodBodyWriter
	{
		...
		protected InteropMethodBodyWriter(MethodReference interopMethod, InteropMarshaler marshaler)
		{
			...
			this._parameters = new MarshaledParameter[this._interopMethod.Parameters.Count];
			for (int i = 0; i < this._interopMethod.Parameters.Count; i++)
			{
				ParameterDefinition parameterDefinition = this._interopMethod.Parameters[i];
				TypeReference parameterType = this._typeResolver.Resolve(parameterDefinition.ParameterType);
				this._parameters[i] = new MarshaledParameter(parameterDefinition.Name, InteropMethodBodyWriter.Naming.ForParameterName(parameterDefinition), parameterType, parameterDefinition.MarshalInfo, parameterDefinition.IsIn, parameterDefinition.IsOut);
			}
		}
		...
		private string WriteMarshalInputParameter(CppCodeWriter writer, MarshaledParameter parameter, IRuntimeMetadataAccess metadataAccess)
		{
			...
			{
				result = this._marshaler.WriteMarshalEmptyInputParameter(writer, parameter, metadataAccess);
			}
			return result;
		}
		...
		protected string GetMethodNameInGeneratedCode()
		{
		...
		{
			TypeReference parameterType = parameter.ParameterType;
			return (parameter.IsOut && !parameterType.IsValueType()) || ((!parameter.IsIn || parameter.IsOut) && (parameter.ParameterType.IsByReference || (parameterType.MetadataType == MetadataType.Class && parameterType.FullName == "System.Text.StringBuilder")));
		}
