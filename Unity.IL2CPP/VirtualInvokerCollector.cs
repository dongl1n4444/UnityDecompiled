		...
		public void Write(NPath path)
		{
			using (SourceCodeWriter sourceCodeWriter = new SourceCodeWriter(path))
			{
				...
				select m)
				{
					...
					if (this._processGenerics)
					{
						sourceCodeWriter.WriteLine("il2cpp_codegen_get_generic_virtual_invoke_data(method, obj, &invokeData);");
					}
					else
					{
						sourceCodeWriter.WriteLine("il2cpp_codegen_get_virtual_invoke_data (slot, obj, &invokeData);");
					}
