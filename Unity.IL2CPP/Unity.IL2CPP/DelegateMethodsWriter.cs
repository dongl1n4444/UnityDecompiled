...
namespace Unity.IL2CPP
{
	public class DelegateMethodsWriter
	{
		...
		private void WriteMethodBodyForInvoke(MethodReference method)
		{
			...
			this.WriteInvocations(method, parametersOnlyName);
		}

		private void WriteInvocations(MethodReference method, List<string> parametersOnlyName)
		{
			...
			if (parametersOnlyName.Count != 0)
			{
				...
				{
					DelegateMethodsWriter.ExpressionForFieldOfThis(this._targetGetterName),
					"___methodIsStatic"
				});
			}
			else
			...
			this.EmitInvocation(method, this._methodPtrGetterName + "()", this._targetGetterName + "()", parametersOnlyName, text, false, true);
			...
			this._writer.BeginBlock();
			this.EmitInvocation(method, this._methodPtrGetterName + "()", this._targetGetterName + "()", parametersOnlyName, text, false, false);
			if (flag)
			{
				this._writer.EndBlock(false);
				this.WriteLine("else", new object[0]);
				this._writer.BeginBlock();
				this.EmitInvocation(method, this._methodPtrGetterName + "()", this._targetGetterName + "()", parametersOnlyName, text, true, false);
			}
			this._writer.EndBlock(false);
		}
		...
		private void EmitInvocation(MethodReference method, string methodPtrFieldName, string targetFieldName, List<string> parametersOnlyName, string methodInfoExpression, bool useFirstArgumentAsThis = false, bool forStatic = false)
		{
			...
			string text2 = string.Format("(({0}){1})", "FunctionPointerType", DelegateMethodsWriter.ExpressionForFieldOfThis(methodPtrFieldName));
			string text3 = string.Format("il2cpp_codegen_get_delegate_this({0})", DelegateMethodsWriter.Naming.ThisParameterName);
		...
		{
			return string.Format("{0}->{1}", DelegateMethodsWriter.Naming.ThisParameterName, targetFieldName);
		}
		...
		private void WriteMethodBodyForBeginInvoke(MethodReference method, IRuntimeMetadataAccess metadataAccess)
		{
			...
			this.WriteLine("return ({0})il2cpp_delegate_begin_invoke((Il2CppDelegate*)__this, __d_args, (Il2CppDelegate*){1}, (Il2CppObject*){2});", new object[]
			{
		...
		private void WriteMethodBodyForDelegateEndInvoke(MethodReference method)
		{
			...
			{
				this.WriteLine("il2cpp_delegate_end_invoke((Il2CppAsyncResult*) {0}, {1});", new object[]
				{
			...
			{
				this.WriteLine("Il2CppObject *__result = il2cpp_delegate_end_invoke((Il2CppAsyncResult*) {0}, {1});", new object[]
				{
					DelegateMethodsWriter.Naming.ForParameterName(parameterReference),
					text
				});
