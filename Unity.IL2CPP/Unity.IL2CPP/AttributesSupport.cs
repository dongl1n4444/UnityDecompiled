...
namespace Unity.IL2CPP
{
	public class AttributesSupport
	{
				...
				for (int i = 0; i < constructibleCustomAttributes2.Length; i++)
				{
					...
					using (Collection<CustomAttributeNamedArgument>.Enumerator enumerator4 = customAttribute.Properties.GetEnumerator())
					{
						while (enumerator4.MoveNext())
						{
							...
							MethodDefinition setMethod = source.Single((PropertyDefinition p) => p.Name == property.Name).SetMethod;
			...
			foreach (CustomAttribute current in customAttributes)
			{
				...
				using (Collection<CustomAttributeNamedArgument>.Enumerator enumerator2 = current.Fields.GetEnumerator())
				{
					while (enumerator2.MoveNext())
					{
						...
						{
							AttributesSupport.Naming.ForFieldSetter(source.Single((FieldDefinition p) => p.Name == fieldArgument.Name)),
				...
				using (Collection<CustomAttributeNamedArgument>.Enumerator enumerator3 = current.Properties.GetEnumerator())
				{
					while (enumerator3.MoveNext())
					{
						...
						MethodDefinition setMethod = source2.Single((PropertyDefinition p) => p.Name == propertyArgument.Name).SetMethod;
		...
		private static void WriteStoreArrayElement(CppCodeWriter writer, string varName, TypeReference elementType, int index, string value)
		{
			...
			{
				Emit.StoreArrayElement(varName, index.ToString(), value)
			});
		}
		...
		private static string FormatAttributeValue(CustomAttributeArgument argument, string tempLocalName, DefaultRuntimeMetadataAccess metadataAccess)
		{
			...
			{
				string result = argument.Value.ToString().ToLower();
				return result;
			}
			case MetadataType.Char:
			{
				string result = "'" + argument.Value + "'";
				return result;
			}
			...
			{
				string result = argument.Value.ToString().ToLower() + "LL";
				return result;
			}
			case MetadataType.Single:
			{
				string result = Formatter.StringRepresentationFor((float)argument.Value);
				return result;
			}
			case MetadataType.Double:
			{
				string result = Formatter.StringRepresentationFor((double)argument.Value);
				return result;
			}
			case MetadataType.String:
			{
				string result = AttributesSupport.StringWrapperFor((string)argument.Value);
				return result;
			}
			case MetadataType.ValueType:
				if (argument.Type.Resolve().IsEnum)
				{
					string result = argument.Value.ToString();
					return result;
				}
				break;
			case MetadataType.Class:
			{
				string result = AttributesSupport.TypeWrapperFor((TypeReference)argument.Value, metadataAccess);
				return result;
			}
			...
			throw new NotSupportedException("Unsupported constructor argument metadata type: " + argument.Type.MetadataType);
		}
