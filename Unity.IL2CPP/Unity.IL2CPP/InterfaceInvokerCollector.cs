		...
		public void Write(NPath path)
		{
			using (SourceCodeWriter sourceCodeWriter = new SourceCodeWriter(path))
			{
				...
				select m)
				{
					...
					if (this._processGenerics)
					{
						sourceCodeWriter.WriteLine("il2cpp::vm::Runtime::GetGenericInterfaceInvokeData (method, obj, &invokeData);");
					}
					else if (current.Com)
					{
						sourceCodeWriter.WriteLine("il2cpp::vm::Runtime::GetComInterfaceInvokeData (slot, declaringInterface, obj, &invokeData);");
					}
					else
					{
						sourceCodeWriter.WriteLine("il2cpp::vm::Runtime::GetInterfaceInvokeData (slot, declaringInterface, obj, &invokeData);");
					}
