...
namespace Unity.IL2CPP
{
	public class AssemblyConverter
	{
		...
		protected AssemblyConverter(NPath[] assemblies, NPath outputDir, NPath dataFolder, DotNetProfile dotNetProfile)
		{
			...
			this._assemblyLoader = new AssemblyLoader(searchDirectories, DebuggerOptions.Enabled || CodeGenOptions.EnableSymbolLoading);
		}
		...
		{
			this._assembliesOrderedByDependency = AssemblyConverter.CollectAssembliesRecursive(from path in this._assemblies
		...
		private void PreProcessStage(out InflatedCollectionCollector genericsCollectionCollector, out TypeDefinition[] allTypeDefinitions)
		{
			...
			AssemblyConverter.TypeProviderInitializer.Initialize(this._assembliesOrderedByDependency.Single((AssemblyDefinition ad) => ad.Name.Name == "mscorlib"));
			this._outputDir.EnsureDirectoryExists("");
			if (this._assembliesOrderedByDependency.Any((AssemblyDefinition a) => a.EntryPoint != null))
			{
				this.CreateDriverCpp();
			}
		...
		private void CreateDriverCpp()
		{
			string text = Encoding.UTF8.GetString(ResourceExtraction.Extract("Unity.IL2CPP.driver.cpp", AssemblyPortable.GetExecutingAssemblyPortable()));
			if (PlatformUtils.IsWindows() && Debugger.IsAttached)
			{
				text = text.Replace("//AdditionalIncludesGoHere", "#include \"windows.h\"");
				text = text.Replace("//MessageBoxGoesHereIfNeeded", "MessageBoxA(NULL, \"Attach\", \"Attach\", MB_OK);");
			}
			if (CodeGenOptions.DumpRuntimeStats)
			{
				text = text.Replace("//DumpStatsGoesHereIfNeeded", "il2cpp_stats_dump_to_file(\"runtime-stats.txt\");");
			}
			File.WriteAllBytes(this._outputDir.Combine(new string[]
			{
				"driver.cpp"
			}).ToString(), Encoding.UTF8.GetBytes(text));
		}
