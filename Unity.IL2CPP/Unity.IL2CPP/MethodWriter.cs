...
namespace Unity.IL2CPP
{
	public class MethodWriter
	{
		...
		internal void WriteMethodDeclarationsFor(Func<MethodDefinition, bool> filter)
		{
			...
			{
				MarshalType marshalType = array[j];
				DefaultMarshalInfoWriter defaultMarshalInfoWriter = MarshalDataCollector.MarshalInfoWriterFor(typeDefinition, marshalType, null, false, false);
		...
		internal void WriteMethodDefinitions(IMethodCollector methodCollector)
		{
			...
			for (int j = 0; j < array.Length; j++)
			{
				MarshalType marshalType = array[j];
				DefaultMarshalInfoWriter defaultMarshalInfoWriter = MarshalDataCollector.MarshalInfoWriterFor(typeDefinition, marshalType, null, false, false);
				if (defaultMarshalInfoWriter.HasMarshalFunctions && marshalType == MarshalType.PInvoke)
				{
					methodCollector.AddTypeMarshallingFunctions(typeDefinition);
				}
				defaultMarshalInfoWriter.WriteMarshalFunctionDefinitions(this._writer);
			}
			if (!typeDefinition.IsComOrWindowsRuntimeType())
			{
				if (typeDefinition.Interfaces.Any((TypeReference i) => i.IsComOrWindowsRuntimeInterface()))
		...
		internal static void WriteMethodWithMetadataInitialization(CppCodeWriter writer, string methodSignature, string methodFullName, Action<CppCodeWriter, MetadataUsage, MethodUsage> writeMethodBody, string uniqueIdentifier)
		{
			...
			using (CppCodeWriter cppCodeWriter = new InMemoryCodeWriter())
			{
				...
				{
					cppCodeWriter2.Indent();
					cppCodeWriter.Indent();
					...
					cppCodeWriter2.Dedent();
					cppCodeWriter.Dedent();
		...
		private static void WriteMethodBodyForComOrWindowsRuntimeMethod(MethodReference method, CppCodeWriter writer, IRuntimeMetadataAccess metadataAccess)
		{
			...
			{
				if (methodDefinition.DeclaringType.IsImport)
				{
					MethodWriter.WriteMethodBodyForComObjectConstructor(method, writer, methodDefinition);
				}
				else
				{
					MethodWriter.WriteMethodBodyForWindowsRuntimeObjectConstructor(method, writer, methodDefinition);
				}
			}
		...
		{
			writer.WriteLine("il2cpp_codegen_com_initialize_object({0}, {1}::CLSID);", new object[]
			{
		...
		private static void WriteMethodBodyForWindowsRuntimeObjectConstructor(MethodReference method, CppCodeWriter writer, MethodDefinition methodDefinition)
		{
			...
			if (methodDefinition2.DeclaringType.IsAttribute())
			{
				string arg = string.Format("Cannot construct type '{0}'. Windows Runtime attribute types are not constructable.", methodDefinition2.DeclaringType);
				writer.WriteStatement(Emit.RaiseManagedException(string.Format("il2cpp_codegen_get_invalid_operation_exception(\"{0}\")", arg)));
			}
			writer.WriteStatement(Emit.RaiseManagedException("il2cpp_codegen_get_not_supported_exception(\"Constructing Windows Runtime objects is not supported in IL2CPP yet\")"));
		}
		...
		private static void WriteMethodBodyForComOrWindowsRuntimeFinalizer(MethodDefinition finalizer, CppCodeWriter writer, IRuntimeMetadataAccess metadataAccess)
		{
			...
			{
				foreach (TypeReference current in from i in declaringType.Interfaces
				where i.IsComOrWindowsRuntimeInterface()
				select i)
				{
					MethodWriter.WriteReleaseField(writer, MethodWriter.Naming.ForComTypeInterfaceFieldName(current), null);
				}
			}
			else
			{
				MethodWriter.WriteReleaseField(writer, MethodWriter.Naming.ForIl2CppComObjectIdentityField(), string.Format("il2cpp_codegen_il2cpp_com_object_cleanup({0});", MethodWriter.Naming.ThisParameterName));
			}
			MethodWriter.CallBaseTypeFinalizer(finalizer, writer, metadataAccess);
		}

		private static void WriteReleaseField(CppCodeWriter writer, string fieldName, string onBeforeRelease = null)
		{
			writer.WriteLine("if ({0}->{1} != {2})", new object[]
			{
				MethodWriter.Naming.ThisParameterName,
				fieldName,
				MethodWriter.Naming.Null
			});
			using (new BlockWriter(writer, false))
			{
				if (onBeforeRelease != null)
				{
					writer.WriteLine(onBeforeRelease);
				}
				writer.WriteLine("{0}->{1}->Release();", new object[]
				{
					MethodWriter.Naming.ThisParameterName,
					fieldName
				});
		...
		private static void CallBaseTypeFinalizer(MethodDefinition finalizer, CppCodeWriter writer, IRuntimeMetadataAccess metadataAccess)
		{
			...
			if (methodReference != null)
			{
				...
				string methodCallExpression = MethodBodyWriter.GetMethodCallExpression(finalizer, typeResolver2.Resolve(methodReference), methodReference, typeResolver2, MethodCallType.Normal, metadataAccess, new VTableBuilder(), list, null);
		...
		private static void WriteMethodBodyForDirectComOrWindowsRuntimeCall(MethodReference method, CppCodeWriter writer, IRuntimeMetadataAccess metadataAccess)
		{
			...
			if (((!methodDefinition.DeclaringType.IsInterface) ? method.GetOverridenInterfaceMethod(method.DeclaringType.GetInterfaces()) : method) == null)
			{
		...
		private static void WriteMethodBodyForInternalCall(MethodReference method, CppCodeWriter writer, IRuntimeMetadataAccess metadataAccess)
		{
			...
			string text = (!MethodWriter.ReturnsVoid(methodDefinition)) ? "return " : string.Empty;
			string icall = method.FullName.Substring(method.FullName.IndexOf(" ") + 1);
			string text2 = MethodWriter.IcallMap.ResolveICallFunction(icall);
			if (text2 != null)
			{
		...
		{
			writer.WriteStatement("static bool s_Il2CppMethodIntialized");
			...
			writer.WriteStatement(Emit.Assign("s_Il2CppMethodIntialized", "true"));
