...
namespace Unity.IL2CPP
{
	public class CodeRegistrationWriter
	{
		...
		public static void WriteCodeRegistration(NPath outputDir, IMethodCollectorResults methodCollector, InflatedCollectionCollector genericsCollectionCollector, MethodTables methodPointerTables, TableInfo attributeGeneratorTable, TableInfo guidTable, UnresolvedVirtualsTablesInfo virtualCallTables)
		{
			...
			CodeRegistrationWriter.WriteCodeRegistration(outputDir, invokerTable, methodPointersTable, reversePInvokeWrappersTable, delegateWrappersManagedToNativeTable, marshalingFunctionsTable, ccwMarshalingFunctionsTable, genericMethodPointerTable, attributeGeneratorTable, guidTable, virtualCallTables);
		}
		...
		private static void WriteCodeRegistration(NPath outputDir, TableInfo invokerTable, TableInfo methodPointersTable, TableInfo reversePInvokeWrappersTable, TableInfo delegateWrappersManagedToNativeTable, TableInfo marshalingFunctionsTable, TableInfo ccwMarshalingFunctionsTable, TableInfo genericMethodPointerTable, TableInfo attributeGeneratorTable, TableInfo guidTable, UnresolvedVirtualsTablesInfo virtualCallTables)
		{
			...
			})))
			{
				...
				if (virtualCallTables.MethodPointersInfo.Count > 0)
				{
					sourceCodeWriter.WriteLine("{0} {1}[];", new object[]
					{
						virtualCallTables.MethodPointersInfo.Type,
						virtualCallTables.MethodPointersInfo.Name
					});
				}
				sourceCodeWriter.WriteStructInitializer("const Il2CppCodeRegistration", "g_CodeRegistration", new string[]
				{
					...
					virtualCallTables.MethodPointersInfo.Count.ToString(CultureInfo.InvariantCulture),
					(virtualCallTables.MethodPointersInfo.Count <= 0) ? CodeRegistrationWriter.Naming.Null : virtualCallTables.MethodPointersInfo.Name
				});
			...
			using (TinyProfiler.Section("MarshalingFunctions", "Il2CppMarshalingFunctionsTable.cpp"))
			{
				...
				if (typeMarshalingFunctions.Count > 0)
				{
					...
					})))
					{
						...
						{
							DefaultMarshalInfoWriter defaultMarshalInfoWriter = MarshalDataCollector.MarshalInfoWriterFor(current, MarshalType.PInvoke, null, false, false, false, null);
							sourceCodeWriter.WriteLine("extern \"C\" void {0} ();", new object[]
							{
						...
						CppCodeWriter arg_115_0 = sourceCodeWriter;
						string arg_115_1 = "extern const Il2CppMarshalingFunctions";
						string arg_115_2 = "g_MarshalingFunctions";
						IEnumerable<TypeDefinition> arg_10F_0 = typeMarshalingFunctions;
						...
						arg_115_0.WriteArrayInitializer(arg_115_1, arg_115_2, arg_10F_0.Select(CodeRegistrationWriter.<>f__mg$cache1), true);
		...
		{
			DefaultMarshalInfoWriter defaultMarshalInfoWriter = MarshalDataCollector.MarshalInfoWriterFor(marshalableType, MarshalType.PInvoke, null, false, false, false, null);
		...
		private static TableInfo WriteCCWMarshalingFunctions(NPath outputDir, IMethodCollectorResults methodCollector)
		{
			...
			using (TinyProfiler.Section("CCWMarshalingFunctions", "Il2CppCcwMarshalingFunctionsTable.cpp"))
			{
				...
				if (cCWMarshalingFunctions.Count > 0)
				{
					...
					{
						sourceCodeWriter.AddCodeGenIncludes();
						foreach (TypeDefinition current in cCWMarshalingFunctions)
						{
							sourceCodeWriter.WriteLine("extern \"C\" void {0} ();", new object[]
							{
								CodeRegistrationWriter.Naming.ForCreateComCallableWrapperFunction(current)
							});
						}
						sourceCodeWriter.WriteArrayInitializer("extern const Il2CppMethodPointer", "g_CcwMarshalingFunctions", from t in cCWMarshalingFunctions
						select CodeRegistrationWriter.Naming.ForCreateComCallableWrapperFunction(t), true);
