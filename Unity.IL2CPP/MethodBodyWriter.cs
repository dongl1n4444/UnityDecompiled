...
namespace Unity.IL2CPP
{
	public class MethodBodyWriter
	{
		...
		private static string WriteExpressionAndCastIfNeeded(TypeReference leftType, StackInfo right, SharingType sharingType = SharingType.NonShared)
		{
			...
			{
				ByReferenceType byReferenceType = leftType as ByReferenceType;
				if (byReferenceType != null)
				{
					if (byReferenceType.ElementType.MetadataType == MetadataType.Int64 && right.Type == MethodBodyWriter.TypeProvider.NativeIntTypeReference)
					{
						result = MethodBodyWriter.EmitCastRightCastToLeftType(leftType, right);
						return result;
					}
		...
		private void ProcessInstruction(ExceptionSupport.Node node, InstructionBlock block, ref Instruction ins)
		{
			...
			case Code.Ldelema:
			{
				...
				this._nullCheckSupport.WriteNullCheckIfNeeded(stackInfo6);
				this.PushExpression(typeReference3, this.EmitArrayLoadElementAddress(stackInfo6, stackInfo5.Expression, stackInfo5.Type));
				break;
			}
			case Code.Ldelem_I1:
			...
			case Code.Ldelem_Ref:
			{
				...
				this.LoadElem(array, ArrayUtilities.ArrayElementTypeOf(array.Type), index);
				break;
			}
			case Code.Stelem_I:
			case Code.Stelem_I1:
			...
			case Code.Stelem_Any:
			{
				...
				this.StoreElement(array2, index2, value, false);
				break;
			}
			case Code.Stelem_Ref:
			{
				...
				this.StoreElement(array3, index3, value2, true);
				break;
			}
			...
			case Code.Localloc:
			{
				...
				this._writer.WriteLine(string.Format("{0} {1} = ({0}) alloca({2});", MethodBodyWriter.Naming.ForVariable(pointerType), text2, stackInfo7));
			...
			case Code.Volatile:
				...
				break;
			case Code.Tail:
				MethodBodyWriter.StatsService.RecordTailCall(this._methodDefinition);
				break;
			case Code.Initobj:
			{
				StackInfo stackInfo8 = this._valueStack.Pop();
				ByReferenceType byReferenceType2 = (ByReferenceType)this._typeResolver.Resolve(stackInfo8.Type);
			...
			{
				StackInfo stackInfo9 = this._valueStack.Pop();
				if (stackInfo9.Type.FullName != "System.TypedReference" && stackInfo9.Type.Resolve().Module.Name == "mscorlib")
				{
					throw new InvalidOperationException();
				}
				FieldDefinition field = stackInfo9.Type.Resolve().Fields.Single((FieldDefinition f) => TypeReferenceEqualityComparer.AreEqual(f.FieldType, this.RuntimeTypeHandleTypeReference, TypeComparisonMode.Exact));
				string expression = string.Format("{0}.{1}()", stackInfo9.Expression, MethodBodyWriter.Naming.ForFieldGetter(field));
		...
		private void WriteReturnStatement()
		{
			...
			if (typeReference.MetadataType != MetadataType.Void)
			{
				...
				else if (typeReference.MetadataType == MetadataType.IntPtr && (right.Type.IsSameType(this.NativeIntTypeReference) || right.Type.IsSameType(this.NativeUIntTypeReference)))
				{
				...
				else if (typeReference.MetadataType == MetadataType.UIntPtr && (right.Type.IsSameType(this.NativeIntTypeReference) || right.Type.IsSameType(this.NativeUIntTypeReference)))
				{
			...
			else if (MethodBodyWriter.GenericSharingAnalysis.IsGenericSharingForValueTypesEnabled && this._sharingType == SharingType.Shared)
			{
				...
				if (resolvedMethodToCall.DeclaringType.IsInterface())
				{
					this._writer.WriteLine("int32_t {0} = il2cpp_codegen_class_interface_offset({1}, {2});", new object[]
					{
						...
						text,
						this._runtimeMetadataAccess.TypeInfoFor(methodToCall.DeclaringType)
					});
				}
		...
		private string EmitArrayLoadElementAddress(StackInfo array, string indexExpression, TypeReference indexType)
		{
			...
			{
				result = Emit.LoadArrayElementAddress(array.Expression, this.ArrayIndexerForIntPtr(indexExpression, indexType), this._arrayBoundsCheckSupport.ShouldEmitBoundsChecksForMethod());
			}
			else
			{
				result = Emit.LoadArrayElementAddress(array.Expression, indexExpression, this._arrayBoundsCheckSupport.ShouldEmitBoundsChecksForMethod());
			}
			return result;
		}
		...
		{
			StackInfo originalValue = this._valueStack.Pop();
			TypeReference typeReference = this.CalculateResultTypeForNegate(originalValue.Type);
			if (typeReference.IsSameType(MethodBodyWriter.TypeProvider.SystemIntPtr))
			{
				FieldDefinition field = MethodBodyWriter.TypeProvider.SystemIntPtr.Fields.Single((FieldDefinition f) => f.Name == MethodBodyWriter.Naming.IntPtrValueField);
				string arg = MethodBodyWriter.Naming.ForFieldGetter(field);
				string arg2 = string.Format("{0}.{1}()", originalValue.Expression, arg);
				string stringValue = string.Format("(-(intptr_t){0})", arg2);
				Local local = this.EmitLocalIntPtrWithValue(stringValue);
				this.PushExpression(local.Type, local.Expression);
			}
			else
			{
				this.PushExpression(originalValue.Type, string.Format("(-{0})", MethodBodyWriter.CastTypeIfNeeded(originalValue, typeReference)));
			}
		}
		...
		private string CallExpressionFor(MethodReference callingMethod, MethodReference unresolvedMethodToCall, MethodCallType callType, List<StackInfo> poppedValues, Func<string, string> addUniqueSuffix, bool emitNullCheckForInvocation = true)
		{
			...
			string methodCallExpression = MethodBodyWriter.GetMethodCallExpression(callingMethod, methodReference, unresolvedMethodToCall, typeResolverForMethodToCall, callType, this._runtimeMetadataAccess, this._vTableBuilder, list, this._arrayBoundsCheckSupport.ShouldEmitBoundsChecksForMethod(), addUniqueSuffix);
		...
		internal static string GetMethodCallExpression(MethodReference callingMethod, MethodReference methodToCall, MethodReference unresolvedMethodtoCall, TypeResolver typeResolverForMethodToCall, MethodCallType callType, IRuntimeMetadataAccess runtimeMetadataAccess, VTableBuilder vTableBuilder, IEnumerable<string> argumentArray, bool useArrayBoundsCheck, Func<string, string> addUniqueSuffix = null)
		{
			...
			{
				result = MethodBodyWriter.GetArraySetCall(methodToCall, argumentArray.First<string>(), argumentArray.Skip(1).AggregateWithComma(), useArrayBoundsCheck);
			}
			else if (methodToCall.DeclaringType.IsArray && methodToCall.Name == "Get")
			{
				result = MethodBodyWriter.GetArrayGetCall(methodToCall, argumentArray.First<string>(), argumentArray.Skip(1).AggregateWithComma(), useArrayBoundsCheck);
			}
			else if (methodToCall.DeclaringType.IsArray && methodToCall.Name == "Address")
			{
				result = MethodBodyWriter.GetArrayAddressCall(methodToCall, argumentArray.First<string>(), argumentArray.Skip(1).AggregateWithComma(), useArrayBoundsCheck);
			}
		...
		private static string VirtualCallFor(MethodReference method, MethodReference unresolvedMethod, IEnumerable<string> args, TypeResolver typeResolver, IRuntimeMetadataAccess runtimeMetadataAccess, VTableBuilder vTableBuilder)
		{
			...
			{
				MethodBodyWriter.VirtualCallCollector.AddMethod(method);
			}
		...
		{
			return Emit.Call(string.Format("({0})->{1}", array, MethodBodyWriter.Naming.ForArrayItemAddressGetter(useArrayBoundsCheck)), arguments);
		}
		...
		{
			return Emit.Call(string.Format("({0})->{1}", array, MethodBodyWriter.Naming.ForArrayItemGetter(useArrayBoundsCheck)), arguments);
		}
		...
		{
			return Emit.Call(string.Format("({0})->{1}", array, MethodBodyWriter.Naming.ForArrayItemSetter(useArrayBoundsCheck)), arguments);
		}
		...
		private void WritePointerOverflowCheckUsing64Bits(string op, MethodBodyWriter.OverflowCheck check, StackInfo left, StackInfo right)
		{
			string expressionFor = this.GetExpressionFor(left);
			string expressionFor2 = this.GetExpressionFor(right);
			this.WritePointerOverflowCheckUsing64Bits(op, check, expressionFor, expressionFor2);
		}

		private string GetExpressionFor(StackInfo stackInfo)
		{
			string result;
			if (stackInfo.Type.IsSameType(MethodBodyWriter.TypeProvider.SystemIntPtr))
			{
				FieldDefinition field = MethodBodyWriter.TypeProvider.SystemIntPtr.Fields.Single((FieldDefinition f) => f.Name == MethodBodyWriter.Naming.IntPtrValueField);
				result = string.Format("{0}.{1}()", stackInfo.Expression, MethodBodyWriter.Naming.ForFieldGetter(field));
			}
			else if (stackInfo.Type.IsSameType(MethodBodyWriter.TypeProvider.SystemUIntPtr))
			{
				FieldDefinition field2 = MethodBodyWriter.TypeProvider.SystemUIntPtr.Fields.Single((FieldDefinition f) => f.Name == MethodBodyWriter.Naming.UIntPtrPointerField);
				result = string.Format("{0}.{1}()", stackInfo.Expression, MethodBodyWriter.Naming.ForFieldGetter(field2));
			}
			else
			{
				result = stackInfo.Expression;
			}
			return result;
		}
		...
		private void StoreArg(Instruction ins)
		{
			...
			{
				this.WriteAssignment(MethodBodyWriter.Naming.ThisParameterName, this._typeResolver.ResolveParameterType(this._methodReference, parameterReference), right);
			}
			else
			{
		...
		private void LoadElem(StackInfo array, TypeReference objectType, StackInfo index)
		{
			...
			this._arrayBoundsCheckSupport.RecordArrayBoundsCheckEmitted();
			this.StoreLocalAndPush(objectType, Emit.LoadArrayElement(array.Expression, index2, this._arrayBoundsCheckSupport.ShouldEmitBoundsChecksForMethod()));
		}
		...
		private void StoreElement(StackInfo array, StackInfo index, StackInfo value, bool emitElementTypeCheck)
		{
			...
			this._writer.WriteLine("{0};", new object[]
			{
				Emit.StoreArrayElement(array.Expression, index2, Emit.Cast(type, value.Expression), this._arrayBoundsCheckSupport.ShouldEmitBoundsChecksForMethod())
			});
		}
		...
		private void WriteStloc(int index)
		{
			...
			if (typeReference.IsPointer || typeReference.IsByReference || this.RequiresContravariantCastToStore(typeReference, stackInfo.Type))
			{
				this._writer.WriteLine("{0} = ({1}){2};", new object[]
				{
		...
		{
			return destinationVariable.IsGenericInstance && sourceVariableType.IsGenericInstance && !TypeReferenceEqualityComparer.AreEqual(destinationVariable, sourceVariableType, TypeComparisonMode.Exact);
		}
		...
		private string ConditionalExpressionFor(string cppOperator, MethodBodyWriter.Signedness signedness, bool negate)
		{
			...
			string expressionFor = this.GetExpressionFor(stackInfo2);
			string expressionFor2 = this.GetExpressionFor(stackInfo);
		...
		private TypeReference TypeForComparison(MethodBodyWriter.Signedness signedness, TypeReference type)
		{
			...
			if (!typeReference.IsSameType(MethodBodyWriter.TypeProvider.NativeIntTypeReference))
			{
				...
				case MetadataType.UInt32:
					IL_56:
					if (metadataType == MetadataType.Pointer || metadataType == MetadataType.ByReference)
					{
