...
namespace Unity.IL2CPP.Marshaling.BodyWriters.NativeToManaged
{
	internal class ReversePInvokeMethodBodyWriter : NativeToManagedInteropMethodBodyWriter
	{
		...
		private string GetMethodSignature()
		{
			...
			string marshaledDecoratedTypeName = base.MarshalInfoWriterFor(this.GetMethodReturnType()).MarshaledDecoratedTypeName;
			string callingConvention = this.GetCallingConvention();
			string text2 = (from p in this._parameters
			select string.Format("{0} {1}", base.MarshalInfoWriterFor(p).MarshaledDecoratedTypeName, p.NameInGeneratedCode)).AggregateWithComma();
