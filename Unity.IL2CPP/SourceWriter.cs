...
namespace Unity.IL2CPP
{
	public class SourceWriter
	{
		...
		public static void WriteCollectedMetadata(InflatedCollectionCollector genericsCollectionCollector, ICollection<AssemblyDefinition> usedAssemblies, NPath outputDir, NPath dataFolder, IMetadataCollection metadataCollection, AttributeCollection attributeCollection, VTableBuilder vTableBuilder, IMethodCollectorResults methodCollector)
		{
			...
			select item.Key);
			UnresolvedVirtualsTablesInfo virtualCallTables = SourceWriter.VirtualCallCollector.WriteUnresolvedStubs(outputDir);
			using (TinyProfiler.Section("WriteCodeRegistration", ""))
			{
				CodeRegistrationWriter.WriteCodeRegistration(outputDir, methodCollector, genericsCollectionCollector, methodTables, attributeGeneratorTable, guidTable, virtualCallTables);
			}
			using (TinyProfiler.Section("WriteMetadata", ""))
			{
				MetadataCacheWriter.WriteMetadata(outputDir, dataFolder, genericsCollectionCollector, null, usedAssemblies, methodTables, metadataCollection, attributeCollection, vTableBuilder, methodCollector, virtualCallTables);
			}
		}
		...
		private static void WriteComTypeDeclaration(SourceCodeWriter writer, TypeReference type)
		{
			...
			if (typeDefinition.IsImport && !typeDefinition.IsInterface && !typeDefinition.IsWindowsRuntimeProjection())
			{
				writer.WriteCommentedLine(type.FullName);
				writer.WriteStatement(Emit.Assign("const Il2CppGuid " + SourceWriter.Naming.ForTypeNameOnly(type) + "::CLSID", typeDefinition.GetGuid().ToInitializer()));
			}
			else if (type.IsComOrWindowsRuntimeInterface())
			{
				writer.WriteCommentedLine(type.FullName);
				writer.WriteStatement(Emit.Assign("const Il2CppGuid " + SourceWriter.Naming.ForTypeNameOnly(type) + "::IID", type.GetGuid().ToInitializer()));
			}
