...
namespace Unity.IL2CPP
{
	public class Emit
	{
		...
		{
			return Emit.MultiDimensionalArrayBoundsCheck(string.Format("(uint32_t)({0})->max_length", array), index);
		}
		...
		public static string LoadArrayElement(string array, string index, bool useArrayBoundsCheck)
		{
			...
			{
				array,
				Emit.Naming.ForArrayItemGetter(useArrayBoundsCheck),
		...
		public static string LoadArrayElementAddress(string array, string index, bool useArrayBoundsCheck)
		{
			...
			{
				array,
				Emit.Naming.ForArrayItemAddressGetter(useArrayBoundsCheck),
				Emit.Naming.ForArrayIndexType(),
				index
			});
		}
		...
		public static string StoreArrayElement(string array, string index, string value, bool useArrayBoundsCheck)
		{
			...
			{
				array,
				Emit.Naming.ForArrayItemSetter(useArrayBoundsCheck),
				Emit.Naming.ForArrayIndexType(),
				index,
				value
			});
		...
		{
			string text = Emit.Call("il2cpp_codegen_object_new", metadataAccess.TypeInfoFor(type));
			string result;
			if (type.IsValueType())
			{
				result = text;
			...
			{
				result = Emit.Cast(Emit.Naming.ForTypeNameOnly(type) + "*", text);
			}
			return result;
		}
