...
namespace UnityEditor.Graphs.AnimationStateMachine
{
	internal class Graph : UnityEditor.Graphs.Graph
	{
		...
		private void CreateEdges()
		{
			...
			{
				AnimatorStateTransition animatorStateTransition2 = array2[j];
				if (animatorStateTransition2 != null)
				{
					this.CreateAnyStateEdges(animatorStateTransition2);
				}
			}
			...
			foreach (ChildAnimatorStateMachine current2 in stateMachinesRecursive)
			{
				ChildAnimatorStateMachine[] stateMachines = current2.stateMachine.stateMachines;
				ChildAnimatorStateMachine[] array3 = stateMachines;
				for (int k = 0; k < array3.Length; k++)
				{
					ChildAnimatorStateMachine childAnimatorStateMachine = array3[k];
					AnimatorTransition[] stateMachineTransitions = current2.stateMachine.GetStateMachineTransitions(childAnimatorStateMachine.stateMachine);
					AnimatorTransition[] array4 = stateMachineTransitions;
					for (int l = 0; l < array4.Length; l++)
					{
						AnimatorTransition transition = array4[l];
						this.CreateSelectorEdges(transition, current2.stateMachine, childAnimatorStateMachine.stateMachine);
					}
				}
			}
			AnimatorTransition[] entryTransitions = this.activeStateMachine.entryTransitions;
			AnimatorTransition[] array5 = entryTransitions;
			for (int m = 0; m < array5.Length; m++)
			{
				AnimatorTransition transition2 = array5[m];
		...
		public EdgeInfo GetEdgeInfo(Edge edge)
		{
			...
			{
				result = this.m_ConnectedSlotsCache[Graph.GenerateConnectionKey(edge.fromSlot.node, edge.toSlot.node)];
			}
			return result;
		}
		...
		public Node FindNode(AnimatorState state)
		{
			...
			{
				result = this.m_StateNodeLookup[state];
			}
			else
			{
				...
				else if (this.parentStateMachine && this.HasState(this.rootStateMachine, state, true))
				{
					result = this.m_StateMachineNodeLookup[this.parentStateMachine];
				}
				else
				{
					result = null;
		...
		public Node FindNodeForEdges(AnimatorStateMachine stateMachine)
		{
			...
			{
				result = this.m_EntryNode;
			}
			else
			{
				result = this.FindNode(stateMachine);
			}
			return result;
		}
		...
		public Node FindNode(AnimatorStateMachine stateMachine)
		{
			...
			else if (stateMachine == this.activeStateMachine)
			{
				result = null;
			}
			else if (this.m_StateMachineNodeLookup.ContainsKey(stateMachine))
			{
				result = this.m_StateMachineNodeLookup[stateMachine];
			}
			else
			{
				...
				for (int i = 0; i < stateMachines.Length; i++)
				{
					...
					{
						result = this.m_StateMachineNodeLookup[childAnimatorStateMachine.stateMachine];
						return result;
					}
				}
		...
		private Node FindStateMachineNodeFromState(AnimatorState state, AnimatorStateMachine stateMachine)
		{
			...
			if (this.m_StateMachineProxyLookup.TryGetValue(state, out key))
			{
				...
				while (this.m_StateMachineLookup.TryGetValue(key, out animatorStateMachine))
				{
					...
					{
						result = this.m_StateMachineNodeLookup[key];
						return result;
					}
		...
		{
			StateMachineNode stateMachineNode = this.CreateAndAddNode<StateMachineNode>("", subStateMachine.position);
		...
		{
			StateNode stateNode = this.CreateAndAddNode<StateNode>("", state.position);
