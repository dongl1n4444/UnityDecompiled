...
namespace UnityEditor.Graphs.AnimationStateMachine
{
	internal class SourceNodeTransitionEditor
	{
		...
		public void AcquireController()
		{
			...
			{
				UnityEditor.Animations.AnimatorController expr_3C = this.m_Controller;
				expr_3C.OnAnimatorControllerDirty = (Action)Delegate.Combine(expr_3C.OnAnimatorControllerDirty, new Action(this.ResetTransitionList));
			}
		}
		...
		private void ResetTransitionList()
		{
			if (!this.m_LockList)
			{
				...
				ReorderableList arg_59_0 = this.m_TransitionList;
				if (SourceNodeTransitionEditor.<>f__mg$cache0 == null)
				{
					SourceNodeTransitionEditor.<>f__mg$cache0 = new ReorderableList.HeaderCallbackDelegate(AnimatorTransitionInspectorBase.DrawTransitionHeaderCommon);
				}
				arg_59_0.drawHeaderCallback = SourceNodeTransitionEditor.<>f__mg$cache0;
		...
		public void OnDisable()
		{
			...
			if (this.m_Controller)
			{
				UnityEditor.Animations.AnimatorController expr_37 = this.m_Controller;
				expr_37.OnAnimatorControllerDirty = (Action)Delegate.Remove(expr_37.OnAnimatorControllerDirty, new Action(this.ResetTransitionList));
			}
		}
