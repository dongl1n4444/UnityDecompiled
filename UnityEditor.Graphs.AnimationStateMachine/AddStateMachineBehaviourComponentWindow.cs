	...
	internal class AddStateMachineBehaviourComponentWindow : EditorWindow
	{
		...
		private class Element : IComparable
		{
			...
			public virtual bool OnGUI(bool selected)
			{
				...
				{
					result = rect.Contains(Event.current.mousePosition);
				}
		...
		private class NewScriptElement : AddStateMachineBehaviourComponentWindow.Element
		{
			...
			private string templatePath
			{
				get
				{
					...
					{
						result = Path.Combine(path, "86-C# Script-NewSubStateMachineBehaviourScript.cs.txt");
					}
					else
					{
						result = Path.Combine(path, "86-C# Script-NewStateMachineBehaviourScript.cs.txt");
					}
					return result;
				}
			}
			...
			public override bool OnGUI(bool selected)
			{
				...
				else
				{
					...
					bool flag2 = this.CanCreate();
					if (!flag2 && AddStateMachineBehaviourComponentWindow.className != "")
					{
		...
		private AddStateMachineBehaviourComponentWindow.Element activeElement
		{
			...
			{
				AddStateMachineBehaviourComponentWindow.Element[] activeTree = this.activeTree;
				AddStateMachineBehaviourComponentWindow.Element result;
				...
				{
					result = activeTree[this.selectedIndex];
				}
				return result;
			}
		}
		...
		private GUIContent activeHeader
		{
			get
			{
				...
				{
					result = this.activeElement.content;
				}
				else if (this.activeTree == this.m_SearchTree)
				{
					result = AddStateMachineBehaviourComponentWindow.s_Styles.searchContent;
				}
				else
				{
					result = AddStateMachineBehaviourComponentWindow.s_Styles.behaviourContent;
				}
				return result;
			}
		}
