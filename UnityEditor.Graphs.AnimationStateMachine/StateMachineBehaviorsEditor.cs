...
namespace UnityEditor.Graphs.AnimationStateMachine
{
	internal class StateMachineBehaviorsEditor
	{
		...
		private Editor[] m_BehavioursEditor = null;

		internal static StateMachineBehaviorsEditor.Styles styles
		{
			get
			{
				StateMachineBehaviorsEditor.Styles arg_18_0;
				if ((arg_18_0 = StateMachineBehaviorsEditor.s_Styles) == null)
				{
					arg_18_0 = (StateMachineBehaviorsEditor.s_Styles = new StateMachineBehaviorsEditor.Styles());
				}
				return arg_18_0;
			}
		}
		...
		protected StateMachineBehaviour[] effectiveBehaviours
		{
			get
			{
				...
				if (this.state)
				{
					StateMachineBehaviour[] array = (!this.m_ControllerContext) ? this.state.behaviours : this.m_ControllerContext.GetStateEffectiveBehaviours(this.state, this.m_LayerIndexContext);
					result = array;
				}
				else if (this.stateMachine)
				{
					result = this.stateMachine.behaviours;
				}
				else
				{
					result = null;
		...
		protected void AddBehaviourButton()
		{
			...
			if (flag2)
			{
				if (AddStateMachineBehaviourComponentWindow.Show(rect, this.m_ControllerContext, this.m_LayerIndexContext, new UnityEngine.Object[]
				{
					this.m_Target
				}))
				{
		...
		protected MonoScript GetBehaviourMonoScript(int index)
		{
			...
			if (this.state)
			{
				result = ((!(this.m_ControllerContext != null)) ? this.state.GetBehaviourMonoScript(index) : this.m_ControllerContext.GetBehaviourMonoScript(this.state, this.m_LayerIndexContext, index));
			}
			else if (this.stateMachine)
			{
				result = this.stateMachine.GetBehaviourMonoScript(index);
			}
			else
			{
				result = null;
