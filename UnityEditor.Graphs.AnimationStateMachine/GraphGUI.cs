	...
	internal class GraphGUI : UnityEditor.Graphs.GraphGUI
	{
		...
		private void SetHoveredStateMachine()
		{
			...
			using (List<UnityEditor.Graphs.Node>.Enumerator enumerator = this.m_Graph.nodes.GetEnumerator())
			{
				while (enumerator.MoveNext())
				{
					...
					if (stateMachineNode)
					{
						if (stateMachineNode.position.Contains(mousePosition) && !this.selection.Contains(stateMachineNode))
						{
		...
		private void HandleObjectDragging()
		{
			...
			case EventType.Repaint:
				...
				{
					if (this.m_HoveredStateMachineNode && !this.selection.Contains(this.m_HoveredStateMachineNode) && !this.SelectionOnlyUpNode())
					{
				...
				if (controller.node.IsValid())
				{
					...
					if (!(this.m_LiveLinkInfo.currentState == null))
					{
						...
						if (this.tool.autoLiveLink)
						{
							...
							if (this.m_LiveLinkInfo.currentState != null && this.m_LiveLinkInfo.nextState != null)
							{
								if ((double)this.m_LiveLinkInfo.transitionInfo.normalizedTime > 0.5 || animatorTransitionInfo.anyState)
								{
			...
			if (type != EventType.KeyDown)
			{
				if (type != EventType.ValidateCommand)
				{
					if (type == EventType.ExecuteCommand)
					{
						...
						{
							if (this.CopySelectionToPasteboard())
							{
		...
		private void AddStateFromNewBlendTreeCallback(object data)
		{
			...
			arg_94_0.blendParameter = defaultBlendTreeParameter;
