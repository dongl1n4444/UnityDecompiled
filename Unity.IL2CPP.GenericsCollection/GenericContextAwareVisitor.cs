...
namespace Unity.IL2CPP.GenericsCollection
{
	public class GenericContextAwareVisitor : Visitor
	{
		...
		private static void ProcessHardcodedDependencies(GenericInstanceType type, InflatedCollectionCollector generics, GenericInstanceMethod contextMethod)
		{
			...
			GenericContextAwareVisitor.AddNullableComparerIfNeeded(type, generics, contextMethod, mainModule);
		}
		...
		{
			TypeDefinition type2 = corlib.GetType("System.Collections.Generic", "ObjectComparer`1");
			TypeDefinition type3 = corlib.GetType("System", "Nullable`1");
			TypeDefinition type4 = corlib.GetType("System", "IComparable`1");
			TypeDefinition type5 = corlib.GetType("System.Collections.Generic", "NullableComparer`1");
			TypeDefinition typeDefinition = type.Resolve();
			if (typeDefinition == type2)
			{
				if (type.GenericArguments[0].Resolve() == type3)
				{
					GenericInstanceType genericInstanceType = (GenericInstanceType)type.GenericArguments[0];
					TypeReference typeReference = genericInstanceType.GenericArguments[0];
					GenericInstanceType genericElementComparisonInterface = type4.MakeGenericInstanceType(new TypeReference[]
					{
						typeReference
					});
					...
					{
						GenericInstanceType type6 = type5.MakeGenericInstanceType(new TypeReference[]
						{
							typeReference
						});
						GenericContextAwareVisitor.ProcessGenericType(type6, generics, contextMethod);
					}
				}
			}
		}
